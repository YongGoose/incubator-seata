    ...
    public void test_decompress_with_len_illegal() {
        Assertions.assertThrows(
                ...
                () -> {
                    ...
                    // 4*1024*1024 + 1
                    byte[] frameContentSize =
                            new byte[] {(byte) 0x00, (byte) 0x40, (byte) 0x00, (byte) 0x01};
                    byte[] frameContent =
                            new byte[frameHeaderDescriptor.length + frameContentSize.length];
                    System.arraycopy(
