...
public class SpringBootConfigurationProvider implements ExtConfigurationProvider {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(SpringBootConfigurationProvider.class);

    private static final String INTERCEPT_METHOD_PREFIX = "get";

    private static final Map<String, Object> PROPERTY_BEAN_INSTANCE_MAP =
                        ...
                        (MethodInterceptor)
                                (proxy, method, args, methodProxy) -> {
                                    if (method.getName().startsWith(INTERCEPT_METHOD_PREFIX)
                                        ...
                                        Class<?> dataType =
                                                ReflectionUtil.getWrappedClass(
                                        ...
                                        if (result == null) {
                                            ...
                                            if (args.length > 1) {
                                                ...
                                                if (result != null) {
                                                    // See Configuration#getConfig(String dataId,
                                                    // long timeoutMills)
                                                    if (dataType.isAssignableFrom(
                                                            result.getClass())) {
                                                        return result;
                                                    } else {
                                                        result = null;
                                            ...
                                            try {
                                                ...
                                                LOGGER.error(
                                                        "Get config '{}' default value from the"
    ...
    private Object getDefaultValueFromPropertyObject(String dataId) throws IllegalAccessException {
        ...
        Objects.requireNonNull(
                propertyObj,
                () -> "Instantiate the property object fail: " + propertyClass.getName());
    ...
    private Object getDefaultValueFromPropertyObject(Object propertyObj, String fieldName)
        ...
        Optional<Field> fieldOptional =
                Stream.of(propertyObj.getClass().getDeclaredFields())
        ...
        if (rawDataId.endsWith(SPECIAL_KEY_GROUPLIST)) {
            String suffix =
                    StringUtils.removeStart(
                            StringUtils.removeEnd(rawDataId, DOT + SPECIAL_KEY_GROUPLIST),
        ...
        if (value == null) {
            value =
                    environment.getProperty(
                            org.apache.seata.common.util.StringUtils.hump2Line(dataId), dataType);
        }
        if (value == null) {
            ...
            if (dataId.startsWith(grouplistPrefix)) {
                ...
                String oldGrouplistDataId =
                        SERVICE_PREFIX + DOT + vgroup + DOT + SPECIAL_KEY_GROUPLIST;
