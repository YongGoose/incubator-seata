...
public abstract class AbstractResourceManager implements ResourceManager {
    ...
    private static int appDataErrSize =
            CONFIG.getInt(
                    ConfigurationKeys.RM_APPLICATION_DATA_SIZE_LIMIT,
        ...
        try {
            ...
            if (response.getResultCode() == ResultCode.Failed) {
                throw new RmTransactionException(
                        ...
                        String.format(
                                "branch register failed, xid: %s, errMsg: %s ",
                                xid, response.getMsg()));
            }
            ...
            throw new RmTransactionException(
                    TransactionExceptionCode.IO, "branch register timeout, xid:" + xid, toe);
        } catch (RuntimeException rex) {
            throw new RmTransactionException(
                    TransactionExceptionCode.BranchRegisterFailed,
                    "branch register exception, xid:" + xid,
                    rex);
        }
    }
        ...
        try {
            ...
            if (response.getResultCode() == ResultCode.Failed) {
                throw new RmTransactionException(
                        response.getTransactionExceptionCode(),
                        String.format(
                                "branch report failed, xid: %s, errMsg: %s ",
                                xid, response.getMsg()));
            }
        } catch (TimeoutException toe) {
            throw new RmTransactionException(
                    TransactionExceptionCode.IO, "branch report timeout, xid:" + xid, toe);
        } catch (RuntimeException rex) {
            throw new RmTransactionException(
                    TransactionExceptionCode.BranchReportFailed,
                    "branch report exception, xid:" + xid,
                    rex);
        }
    }
