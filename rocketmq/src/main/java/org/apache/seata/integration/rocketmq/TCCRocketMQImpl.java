...
public class TCCRocketMQImpl implements TCCRocketMQ {
    ...
    public SendResult prepare(Message message, long timeout) throws MQClientException {
        ...
        SendResult sendResult =
                producer.doSendMessageInTransaction(
                        message, timeout, context.getXid(), context.getBranchId());
    ...
    public boolean commit(BusinessActionContext context)
        ...
        if (checkMqStatus(message, sendResult)) {
            throw new TransactionException(
                    "TCCRocketMQ commit but cannot find message or sendResult");
        }
        this.producerImpl.endTransaction(
                message, sendResult, LocalTransactionState.COMMIT_MESSAGE, null);
        LOGGER.info(
                "RocketMQ message send commit, xid = {}, branchId = {}",
                context.getXid(),
                context.getBranchId());
        return true;
    }
    ...
    public boolean rollback(BusinessActionContext context)
        ...
        this.producerImpl.endTransaction(
                message, sendResult, LocalTransactionState.ROLLBACK_MESSAGE, null);
        LOGGER.info(
                "RocketMQ message send rollback, xid = {}, branchId = {}",
                context.getXid(),
                context.getBranchId());
        return true;
    }
    ...
    private static boolean checkMqStatus(Message message, SendResult sendResult) {
        boolean empty =
                message == null
                        || sendResult == null
                        || (StringUtils.isBlank(sendResult.getOffsetMsgId())
                                && StringUtils.isBlank(sendResult.getMsgId()));
        if (empty) {
