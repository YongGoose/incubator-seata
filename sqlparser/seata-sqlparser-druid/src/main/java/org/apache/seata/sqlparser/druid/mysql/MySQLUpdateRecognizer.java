 ...
 *
 */
public class MySQLUpdateRecognizer extends BaseMySQLRecognizer
        implements SQLUpdateRecognizer, JoinRecognizer {
        ...
        if (tableSource instanceof SQLExprTableSource) {
            ...
            if (joinTables.toString().split(MULTI_TABLE_NAME_SEPERATOR).length < minTableNum + 1) {
                throw new ShouldNeverHappenException(
                        "should get at least two table name for update join table source:"
                                + tableSource.toString());
            }
            // will return union table view name and single table names which linked by "#", like t1
            // inner join t2 on t1.id = t2.id#t1#t2
    ...
    private String visitTableName(SQLExprTableSource tableSource) {
        ...
        MySqlOutputVisitor visitor =
                new MySqlOutputVisitor(tableName) {
