...
class DefaultDruidLoader implements DruidLoader {
    ...
    private DefaultDruidLoader(String druidLocation) {
        ...
        if (url != null) {
            ...
            // try to find druid in class code source if it's not in classpath which should usually
            // happen in unit test
            Class<?> clazz = null;
            try {
                clazz =
                        DefaultDruidLoader.class
                                .getClassLoader()
                                .loadClass("com.alibaba.druid.util.StringUtils");
