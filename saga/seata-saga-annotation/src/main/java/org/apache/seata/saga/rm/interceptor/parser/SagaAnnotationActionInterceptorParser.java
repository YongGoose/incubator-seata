...
public class SagaAnnotationActionInterceptorParser implements InterfaceParser {
    ...
    public ProxyInvocationHandler parserInterfaceToProxy(Object target, String objectName) {
        Map<Method, Class<?>> methodClassMap =
                ReflectionUtil.findMatchMethodClazzMap(
                        target.getClass(),
                        method -> method.isAnnotationPresent(getAnnotationClass()));
    ...
    private void registerResource(Object target, Map<Method, Class<?>> methodClassMap) {
        try {
            for (Map.Entry<Method, Class<?>> methodClassEntry : methodClassMap.entrySet()) {
                ...
                if (annotation != null) {
                    Resource resource =
                            createResource(target, methodClassEntry.getValue(), annotation);
                    // registry resource
    ...
    protected Resource createResource(
            Object targetBean, Class<?> serviceClass, Annotation annotation)
            throws NoSuchMethodException {
        CompensationBusinessAction compensationBusinessAction =
                (CompensationBusinessAction) annotation;
        ...
        sagaAnnotationResource.setCompensationMethodName(
                compensationBusinessAction.compensationMethod());
        ...
        sagaAnnotationResource.setCompensationArgsClasses(
                compensationBusinessAction.compensationArgsClasses());
        sagaAnnotationResource.setPhaseTwoCompensationKeys(
                ActionContextUtil.getTwoPhaseArgs(
                        sagaAnnotationResource.getCompensationMethod(),
                        compensationBusinessAction.compensationArgsClasses()));
