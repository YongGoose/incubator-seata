        ...
        if (oldResource != null) {
            ...
            if (newResourceBean != oldResourceBean) {
                throw new RepeatRegistrationException(
                        String.format(
                                "Same SagaAnnotation resource name <%s> between method1 <%s> of"
                                        + " class1 <%s> and method2 <%s> of class2 <%s>, should be"
                                        + " unique",
                                resourceId,
    ...
    public BranchStatus branchRollback(
        ...
        if (resource == null) {
            throw new ShouldNeverHappenException(
                    String.format(
                            "SagaAnnotation resource is not exist, resourceId: %s", resourceId));
        }
        ...
        if (targetBean == null || compensationMethod == null) {
            throw new ShouldNeverHappenException(
                    String.format(
                            "SagaAnnotation resource is not available, resourceId: %s",
                            resourceId));
        }
        ...
        try {
            ...
            LOGGER.info(
                    "SagaAnnotation resource rollback result : {}, xid: {}, branchId: {},"
                            + " resourceId: {}",
                    result,
                    xid,
                    branchId,
            ...
            String msg =
                    String.format(
                            "rollback SagaAnnotation resource error, resourceId: %s, xid: %s.",
                            resourceId, xid);
