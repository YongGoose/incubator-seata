    ...
    public void preProcess(ProcessContext context) throws EngineExecutionException {
        ...
        if (context.hasVariable(DomainConstants.VAR_NAME_IS_LOOP_STATE)) {
            ...
            if (context.hasVariable(DomainConstants.VAR_NAME_CURRENT_COMPEN_TRIGGER_STATE)) {
                ...
                AbstractTaskState compensateState =
                        (AbstractTaskState)
                                stateToBeCompensated
                                        .getStateMachineInstance()
                                        .getStateMachine()
                                        .getState(
                                                EngineUtils.getOriginStateName(
                                                        stateToBeCompensated));
            ...
            copyContextVariables.put(
                    loop.getElementVariableName(), iterator(collection, loopCounter));
            ((HierarchicalProcessContext) context)
                    .setVariableLocally(
                            DomainConstants.VAR_NAME_STATEMACHINE_CONTEXT, copyContextVariables);
        }
    }
