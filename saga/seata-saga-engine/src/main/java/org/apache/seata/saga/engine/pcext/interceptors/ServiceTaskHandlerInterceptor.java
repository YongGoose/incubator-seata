...
public class ServiceTaskHandlerInterceptor implements StateHandlerInterceptor {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(ServiceTaskHandlerInterceptor.class);
    ...
    public void preProcess(ProcessContext context) throws EngineExecutionException {
        ...
        if (EngineUtils.isTimeout(
            ...
            String message =
                    "Saga Transaction [stateMachineInstanceId:"
        ...
        if (contextVariables != null) {
            try {
                ...
                String message =
                        "Task ["
                                + state.getName()
                                + "] input parameters assign failed, please check 'Input'"
                                + " expression:"
                                + e.getMessage();
                ...
                EngineExecutionException exception =
                        ExceptionUtils.createEngineExecutionException(
                                e,
                                FrameworkErrorCode.VariablesAssignError,
                                message,
                                stateMachineInstance,
                                state.getName());
        ...
        if (context.hasVariable(DomainConstants.VAR_NAME_IS_LOOP_STATE) && !isForCompensation) {
            ...
            StateInstance lastRetriedStateInstance =
                    LoopTaskUtils.findOutLastRetriedStateInstance(
                            stateMachineInstance, stateInstance.getName());
            ...
            stateInstance.setStateIdRetriedFor(
                    (String)
                            context.getVariable(
                                    state.getName()
                                            + DomainConstants.VAR_NAME_RETRIED_STATE_INST_ID));
        }
        ...
        if (isForCompensation) {
            ...
            if (stateToBeCompensated != null) {
                ...
                LOGGER.error(
                        "Compensation State[{}] has no state to compensate, maybe this is a bug.",
                        state.getName());
            }
        ...
        if (stateMachineInstance.getStateMachine().isPersist()
            ...
            try {
                ...
                String message =
                        "Record state["
                                + state.getName()
                                + "] started failed, stateMachineInstance["
                                + stateMachineInstance.getId()
                                + "], Reason: "
                                + e.getMessage();
                ...
                EngineExecutionException exception =
                        ExceptionUtils.createEngineExecutionException(
                                e,
                                FrameworkErrorCode.ExceptionCaught,
                                message,
                                stateMachineInstance,
                                state.getName());
        ...
        if (StringUtils.isEmpty(stateInstance.getId())) {
            stateInstance.setId(
                    stateMachineConfig
                            .getSeqGenerator()
                            .generate(DomainConstants.SEQ_ENTITY_STATE_INST));
        }
        stateMachineInstance.putStateInstance(stateInstance.getId(), stateInstance);
        ((HierarchicalProcessContext) context)
                .setVariableLocally(DomainConstants.VAR_NAME_STATE_INST, stateInstance);
    }
    ...
    public void postProcess(ProcessContext context, Exception exp) throws EngineExecutionException {
        ...
        StateInstance stateInstance =
                (StateInstance) context.getVariable(DomainConstants.VAR_NAME_STATE_INST);
        ...
        if (ExecutionStatus.SU.equals(stateInstance.getStatus()) && exp != null) {
            ...
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info(
                        "Although an exception occurs, the execution status map to SU, and the"
                                + " exception is ignored when the execution status decision.");
            }
        ...
        if (serviceOutputParams != null) {
            try {
                ...
                String message =
                        "Task ["
                                + state.getName()
                                + "] output parameters assign failed, please check 'Output'"
                                + " expression:"
                                + e.getMessage();
                ...
                if (stateMachineInstance.getStateMachine().isPersist()
                    ...
                    stateMachineConfig
                            .getStateLogStore()
                            .recordStateFinished(stateInstance, context);
                }
        ...
        if (exp != null
            ...
            // If there is an exception and there is no catch, need to exit the state machine to
            // execute.
    ...
    private void decideExecutionStatus(
        ...
        if (CollectionUtils.isNotEmpty(statusMatchList)) {
            if (state.isAsync()) {
                if (LOGGER.isWarnEnabled()) {
                    LOGGER.warn(
                            "Service[{}.{}] is execute asynchronously, null return value collected,"
                                    + " so user defined Status Matching skipped. stateName: {},"
                                    + " branchId: {}",
                ...
                if (exp == null
                    ...
                    EngineExecutionException exception =
                            new EngineExecutionException(
                                    "State ["
                                            + state.getName()
                                            + "] execute finished, but cannot matching status,"
                                            + " please check its status manually",
                                    FrameworkErrorCode.NoMatchedStatus);
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug(
                                "State[{}] execute finish with status[{}]",
                                state.getName(),
        ...
        if (stateInstance.getStatus() == null
                || ExecutionStatus.RU.equals(stateInstance.getStatus())) {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(
                    "State[{}] finish with status[{}]", state.getName(), stateInstance.getStatus());
        }
    }
}
