...
public class TaskStateRouter implements StateRouter {
    ...
    public Instruction route(ProcessContext context, State state) throws EngineExecutionException {
        ...
        if (stateInstruction.isEnd()) {
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info(
                        "StateInstruction is ended, Stop the StateMachine executing."
        ...
        // The current CompensationTriggerState can mark the compensation process is started and
        ...
        // There is an exception route, indicating that an exception is thrown, and the exception
        // route is prioritized.
        String next =
                (String) context.getVariable(DomainConstants.VAR_NAME_CURRENT_EXCEPTION_ROUTE);
        ...
        if (!StringUtils.hasLength(next)
                && context.hasVariable(DomainConstants.VAR_NAME_CURRENT_CHOICE)) {
    ...
    private Instruction compensateRoute(ProcessContext context, State compensationTriggerState) {
        ...
        if (Boolean.TRUE.equals(
                context.getVariable(DomainConstants.VAR_NAME_FIRST_COMPENSATION_STATE_STARTED))) {

            Exception exception =
                    (Exception) context.getVariable(DomainConstants.VAR_NAME_CURRENT_EXCEPTION);
            if (exception != null) {
        ...
        if (!stateStackToBeCompensated.isEmpty()) {
            ...
            StateMachine stateMachine =
                    (StateMachine) context.getVariable(DomainConstants.VAR_NAME_STATEMACHINE);
            State state =
                    stateMachine.getState(EngineUtils.getOriginStateName(stateToBeCompensated));
                    ...
                    ((HierarchicalProcessContext) context)
                            .setVariableLocally(
                                    compensateState.getName()
                                            + DomainConstants.VAR_NAME_SUB_MACHINE_PARENT_ID,
