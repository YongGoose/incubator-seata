    ...
    public StateMachine getStateMachineById(String stateMachineId) {
        Item item =
                CollectionUtils.computeIfAbsent(
                        stateMachineMapById, stateMachineId, key -> new Item());
            ...
            synchronized (item) {
                if (item.getValue() == null) {
                    ...
                    if (stateMachine != null) {
                        StateMachine parsedStatMachine =
                                StateMachineParserFactory.getStateMachineParser(jsonParserName)
                                        .parse(stateMachine.getContent());
                        if (parsedStatMachine == null) {
                            throw new RuntimeException(
                                    "Parse State Language failed, stateMachineId:"
                                            + stateMachine.getId()
                                            + ", name:"
                        ...
                        stateMachineMapById.put(
                                stateMachine.getName() + "_" + stateMachine.getTenantId(), item);
                    }
                }
            }
        }
    ...
    public StateMachine getStateMachine(String stateMachineName, String tenantId) {
        Item item =
                CollectionUtils.computeIfAbsent(
                        ...
                        stateMachineName + "_" + tenantId,
                        key -> new Item());
        if (item.getValue() == null && stateLangStore != null) {
            synchronized (item) {
                if (item.getValue() == null) {
                    StateMachine stateMachine =
                            stateLangStore.getLastVersionStateMachine(stateMachineName, tenantId);
                    if (stateMachine != null) {
                        StateMachine parsedStatMachine =
                                StateMachineParserFactory.getStateMachineParser(jsonParserName)
                                        .parse(stateMachine.getContent());
        ...
        if (stateLangStore != null) {
            ...
            if (oldStateMachine != null) {
                ...
                if (Arrays.equals(bytesContent, oldBytesContent)
                    ...
                    LOGGER.info(
                            "StateMachine[{}] is already exist a same version", stateMachineName);
