...
public class ScriptTaskStateHandler implements StateHandler, InterceptableStateHandler {
    ...
    public void process(ProcessContext context) throws EngineExecutionException {
        ...
        try {
            List<Object> input =
                    (List<Object>) context.getVariable(DomainConstants.VAR_NAME_INPUT_PARAMS);

            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                        ">>>>>>>>>>>>>>>>>>>>>> Start to execute ScriptTaskState[{}],"
                                + " ScriptType[{}], Input:{}",
                        state.getName(),
                        scriptType,
                        input);
            ...
            ScriptEngine scriptEngine =
                    getScriptEngineFromCache(
                            scriptType, stateMachineConfig.getScriptEngineManager());
            if (scriptEngine == null) {
                throw new EngineExecutionException(
                        "No such ScriptType[" + scriptType + "]",
                ...
                if (inputExpMap.size() > 0) {
                    ...
                    for (String property : inputExpMap.keySet()) {
                        if (inputMap != null && inputMap.containsKey(property)) {
                            ...
                            // if we do not bind the null value property, groovy will throw
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                        "<<<<<<<<<<<<<<<<<<<<<< ScriptTaskState[{}], ScriptType[{}], Execute"
                                + " finish. result: {}",
                        state.getName(),
                        scriptType,
                        result);
            ...
            LOGGER.error(
                    "<<<<<<<<<<<<<<<<<<<<<< ScriptTaskState[{}], ScriptTaskState[{}] Execute"
                            + " failed.",
                    state.getName(),
                    scriptType,
                    e);
            ...
            ((HierarchicalProcessContext) context)
                    .setVariableLocally(DomainConstants.VAR_NAME_CURRENT_EXCEPTION, e);

            EngineUtils.handleException(context, state, e);
        }
    }

    protected ScriptEngine getScriptEngineFromCache(
            String scriptType, ScriptEngineManager scriptEngineManager) {
