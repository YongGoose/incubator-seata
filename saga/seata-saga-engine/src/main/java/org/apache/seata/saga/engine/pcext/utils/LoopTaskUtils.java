...
public class LoopTaskUtils {
        ...
        if (matchLoop(currentState)) {
            ...
            if (null != taskState.getLoop()) {
                ...
                if (StringUtils.isNotBlank(collectionName)) {
                    Object expression =
                            ParameterUtils.createValueExpression(
                                    stateMachineConfig.getExpressionResolver(), collectionName);
                    Object collection =
                            ParameterUtils.getValue(
                                    expression, stateMachineInstance.getContext(), null);
                ...
                LOGGER.warn(
                        "State [{}] loop collection param [{}] invalid",
    ...
    public static ProcessContext createLoopEventContext(ProcessContext context, int loopCounter) {
        ...
        copyContext.setVariableLocally(
                DomainConstants.LOOP_COUNTER,
                loopCounter >= 0 ? loopCounter : acquireNextLoopCounter(context));
    ...
    public static StateInstance findOutLastNeedForwardStateInstance(ProcessContext context) {
        StateMachineInstance stateMachineInstance =
                (StateMachineInstance)
                        context.getVariable(DomainConstants.VAR_NAME_STATEMACHINE_INST);
        StateInstance lastForwardState =
        ...
        for (int i = actList.size() - 1; i >= 0; i--) {
            ...
            if (EngineUtils.getOriginStateName(stateInstance)
                            .equals(EngineUtils.getOriginStateName(lastForwardState))
            ...
            synchronized (currentLoopContext) {
                if (!currentLoopContext.isCompletionConditionSatisfied()) {
                    ...
                    stateMachineContext.put(
                            DomainConstants.NUMBER_OF_INSTANCES, (double) nrOfInstances);
                    stateMachineContext.put(
                            DomainConstants.NUMBER_OF_ACTIVE_INSTANCES,
                            (double) nrOfActiveInstances);
                    stateMachineContext.put(
                            DomainConstants.NUMBER_OF_COMPLETED_INSTANCES,
     ...
     * @param context the process context
     */
    public static void putContextToParent(
            ProcessContext context, List<ProcessContext> subContextList, State state) {
        ...
        if (CollectionUtils.isNotEmpty(subContextList)) {
            ...
            for (ProcessContext subProcessContext : subContextList) {
                StateInstance stateInstance =
                        (StateInstance)
                                subProcessContext.getVariable(DomainConstants.VAR_NAME_STATE_INST);
    ...
    public static boolean isForSubStateMachineForward(ProcessContext context) {
        ...
        StateInstance lastRetriedStateInstance =
                LoopTaskUtils.findOutLastRetriedStateInstance(
                        stateMachineInstance,
                        LoopTaskUtils.generateLoopStateName(context, instruction.getStateName()));
        ...
        if (null != lastRetriedStateInstance
            ...
            if (ExecutionStatus.UN.equals(subInst.get(0).getCompensationStatus())) {
                throw new ForwardInvalidException(
                        "Last forward execution state instance is SubStateMachine and compensation"
                                + " status is [UN], Operation[forward] denied, stateInstanceId:"
     ...
     */
    public static String decideCurrentExceptionRoute(
            List<ProcessContext> subContextList, StateMachine stateMachine) {
