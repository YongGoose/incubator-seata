...
public class CompensationTriggerStateHandler implements StateHandler {
    ...
    public void process(ProcessContext context) throws EngineExecutionException {
        ...
        if (CollectionUtils.isNotEmpty(stateListToBeCompensated)) {
            ...
            Exception e =
                    (Exception) context.removeVariable(DomainConstants.VAR_NAME_CURRENT_EXCEPTION);
            if (e != null) {
            ...
            // it indicates that the compensation state is automatically initiated in the state
            // machine,
            // and the forward state needs to be changed to the UN state.
            // If the forward status is not the two states, then the compensation operation should
            // be initiated by
            // server recovery,
            ...
            // Record the status of the state machine as "compensating", and the subsequent routing
            // logic will route
