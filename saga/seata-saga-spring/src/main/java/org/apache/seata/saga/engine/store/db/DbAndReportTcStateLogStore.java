...
public class DbAndReportTcStateLogStore extends AbstractStore implements StateLogStore {
    ...
    private static final StateMachineInstanceToStatementForInsert
            STATE_MACHINE_INSTANCE_TO_STATEMENT_FOR_INSERT =
                    new StateMachineInstanceToStatementForInsert();
    private static final StateMachineInstanceToStatementForUpdate
    ...
    private static final ResultSetToStateInstance RESULT_SET_TO_STATE_INSTANCE =
            new ResultSetToStateInstance();
    ...
    public void recordStateMachineStarted(
        ...
        if (machineInstance != null) {
            // if parentId is not null, machineInstance is a SubStateMachine, do not start a new
            // global transaction,
            ...
            try {
                if (StringUtils.isEmpty(machineInstance.getId()) && seqGenerator != null) {
                    machineInstance.setId(
                            seqGenerator.generate(DomainConstants.SEQ_ENTITY_STATE_MACHINE_INST));
                }
                ...
                // save to db
                machineInstance.setSerializedStartParams(
                        paramsSerializer.serialize(machineInstance.getStartParams()));
                int effect =
                        executeUpdate(
                ...
                if (effect < 1) {
                    throw new StoreException(
                            "StateMachineInstance record start error, Xid: "
    ...
    protected void beginTransaction(StateMachineInstance machineInstance, ProcessContext context) {
        if (sagaTransactionalTemplate != null) {
            ...
            transactionInfo.setName(
                    Constants.SAGA_TRANS_NAME_PREFIX + machineInstance.getStateMachine().getName());
            try {
                ...
                if (Boolean.TRUE.equals(
                        context.getVariable(DomainConstants.VAR_NAME_IS_ASYNC_EXECUTION))) {
                    RootContext.unbind();
        ...
        if (machineInstance != null) {
            try {
                ...
                if (ExecutionStatus.SU.equals(machineInstance.getStatus())
                        && machineInstance.getException() != null) {
                    machineInstance.setException(null);
                }

                machineInstance.setSerializedEndParams(
                        paramsSerializer.serialize(machineInstance.getEndParams()));
                machineInstance.setSerializedException(
                        exceptionSerializer.serialize(machineInstance.getException()));
                int effect =
                        executeUpdate(
                ...
                if (effect < 1) {
                    LOGGER.warn(
                            "StateMachineInstance[{}] is recovery by server, skip"
                    ...
                    if (EngineUtils.isTimeout(
                            machineInstance.getGmtUpdated(),
                            stateMachineConfig.getTransOperationTimeout())) {
                        LOGGER.warn(
                                "StateMachineInstance[{}] is execution timeout, skip report"
                                        + " transaction finished to server.",
                                machineInstance.getId());
                    } else if (StringUtils.isEmpty(machineInstance.getParentId())) {
                        // if parentId is not null, machineInstance is a SubStateMachine, do not
                        // report global transaction.
    ...
    protected void reportTransactionFinished(
            StateMachineInstance machineInstance, ProcessContext context) {
        if (sagaTransactionalTemplate != null) {
            ...
            try {
                ...
                LOGGER.error(
                        "Report transaction finish to server error: {}, StateMachine: {}, XID: {},"
                                + " Reason: {}",
                        e.getCode(),
                        machineInstance.getStateMachine().getName(),
                        machineInstance.getId(),
    ...
    public void recordStateStarted(StateInstance stateInstance, ProcessContext context) {
        if (stateInstance != null) {
            ...
            stateInstance.setSerializedInputParams(
                    paramsSerializer.serialize(stateInstance.getInputParams()));
            if (!isUpdateMode) {
                executeUpdate(
        ...
        if (sagaTransactionalTemplate != null) {
            ...
            if (stateMachineConfig instanceof DbStateMachineConfig
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(
                            "sagaBranchRegisterEnable = false, skip register branch. state["
            ...
            try {
                ...
                GlobalTransaction globalTransaction =
                        getGlobalTransaction(machineInstance, context);
                if (globalTransaction == null) {
                    throw new EngineExecutionException(
                            "Global transaction is not exists", FrameworkErrorCode.ObjectNotExists);
                }
    ...
    protected GlobalTransaction getGlobalTransaction(
            StateMachineInstance machineInstance, ProcessContext context)
        ...
        if (globalTransaction == null) {
            ...
            if (StringUtils.isEmpty(parentId)) {
                ...
                xid =
                        parentId.substring(
                                0, parentId.lastIndexOf(DomainConstants.SEPERATOR_PARENT_ID));
            }
    ...
    private String generateRetryStateInstanceId(StateInstance stateInstance) {
        ...
        StateInstance originalStateInst =
                stateInstanceMap.get(stateInstance.getStateIdRetriedFor());
     ...
     * @return
     */
    private String generateCompensateStateInstanceId(
            StateInstance stateInstance, boolean isUpdateMode) {
        ...
        for (int i = 0; i < stateInstance.getStateMachineInstance().getStateList().size(); i++) {
            ...
            if (aStateInstance != stateInstance
                    && originalCompensateStateInstId.equals(
    ...
    public void recordStateFinished(StateInstance stateInstance, ProcessContext context) {
        if (stateInstance != null) {
            ...
            stateInstance.setSerializedOutputParams(
                    paramsSerializer.serialize(stateInstance.getOutputParams()));
            stateInstance.setSerializedException(
                    exceptionSerializer.serialize(stateInstance.getException()));
            executeUpdate(
    ...
    protected void branchReport(StateInstance stateInstance, ProcessContext context) {
        if (sagaTransactionalTemplate != null) {
            ...
            if (stateMachineConfig instanceof DbStateMachineConfig
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(
                            "sagaBranchRegisterEnable = false, skip branch report. state["
                                    + stateInstance.getName()
                                    + "]");
                }
                return;
            ...
            // find out the original state instance, only the original state instance is registered
            // on the server, and its status should
            // be reported.
            ...
            try {
                ...
                sagaTransactionalTemplate.branchReport(
                        ...
                        Long.parseLong(originalStateInst.getId()),
                        branchStatus,
                        null);
            } catch (TransactionException e) {
                LOGGER.error(
                        "Report branch status to server error: {}, StateMachine:{}, StateName:{},"
                                + " XID: {}, branchId: {}, branchStatus:{}, Reason:{} ",
                        e.getCode(),
    ...
    private StateInstance findOutOriginalStateInstanceOfCompensateState(
    ...
    public StateMachineInstance getStateMachineInstance(String stateMachineInstanceId) {
        ...
        List<StateInstance> stateInstanceList =
                queryStateInstanceListByMachineInstanceId(stateMachineInstanceId);
    ...
    private void deserializeParamsAndException(StateMachineInstance stateMachineInstance) {
        ...
        if (StringUtils.hasLength(serializedEndParams)) {
            stateMachineInstance.setEndParams(
                    (Map<String, Object>) paramsSerializer.deserialize(serializedEndParams));
        }
    }
    ...
    public List<StateInstance> queryStateInstanceListByMachineInstanceId(
        ...
        Map<String /* originStateId */, StateInstance /* compensatedState */> compensatedStateMap =
                new HashMap<>();
        Map<String /* originStateId */, StateInstance /* retriedState */> retriedStateMap =
                new HashMap<>();
        for (StateInstance tempStateInstance : stateInstanceList) {
            ...
            if (StringUtils.hasText(tempStateInstance.getStateIdCompensatedFor())) {
                putLastStateToMap(
                        compensatedStateMap,
                        tempStateInstance,
                        tempStateInstance.getStateIdCompensatedFor());
            } else {
                if (StringUtils.hasText(tempStateInstance.getStateIdRetriedFor())) {
                    putLastStateToMap(
                            retriedStateMap,
                            tempStateInstance,
                            tempStateInstance.getStateIdRetriedFor());
                }
    ...
    private void putLastStateToMap(
            Map<String, StateInstance> resultMap, StateInstance newState, String key) {
    ...
    private static class StateMachineInstanceToStatementForInsert
            implements ObjectToStatement<StateMachineInstance> {
        @Override
        public void toStatement(
                StateMachineInstance stateMachineInstance, PreparedStatement statement)
                throws SQLException {
    ...
    private static class StateInstanceToStatementForInsert
            implements ObjectToStatement<StateInstance> {
        @Override
        public void toStatement(StateInstance stateInstance, PreparedStatement statement)
                throws SQLException {
    ...
    private static class StateInstanceToStatementForUpdate
        ...
        public void toStatement(StateInstance stateInstance, PreparedStatement statement)
            ...
            statement.setBytes(
                    2,
                    stateInstance.getException() != null
                            ? (byte[]) stateInstance.getSerializedException()
                            : null);
    ...
    private static class ResultSetToStateMachineInstance
            implements ResultSetToObject<StateMachineInstance> {
        @Override
        public StateMachineInstance toObject(ResultSet resultSet) throws SQLException {
            ...
            if (StringUtils.hasLength(compensationStatusName)) {
                stateMachineInstance.setCompensationStatus(
                        ExecutionStatus.valueOf(compensationStatusName));
            }
