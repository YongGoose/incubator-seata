...
public class DbStateLangStore extends AbstractStore implements StateLangStore {
    ...
    private static final ResultSetToStateMachine RESULT_SET_TO_STATE_MACHINE =
            new ResultSetToStateMachine();

    private static final StateMachineToStatement STATE_MACHINE_TO_STATEMENT =
            new StateMachineToStatement();
    ...
    public StateMachine getStateMachineById(String stateMachineId) {
        return selectOne(
                stateLangStoreSqls.getGetStateMachineByIdSql(dbType),
    ...
    public boolean storeStateMachine(StateMachine stateMachine) {
        return executeUpdate(
                        stateLangStoreSqls.getInsertStateMachineSql(dbType),
                        STATE_MACHINE_TO_STATEMENT,
                        stateMachine)
                > 0;
    }
    ...
    private static class StateMachineToStatement implements ObjectToStatement<StateMachine> {
        @Override
        public void toStatement(StateMachine stateMachine, PreparedStatement statement)
                throws SQLException {
