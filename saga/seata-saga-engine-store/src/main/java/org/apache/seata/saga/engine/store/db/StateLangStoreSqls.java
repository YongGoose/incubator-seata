...
public class StateLangStoreSqls {
    ...
    private static final String STATE_MACHINE_FIELDS =
            "id, tenant_id, app_name, name, status, gmt_create, ver, type, content,"
                    + " recover_strategy, comment_";
    ...
    private static final String INSERT_STATE_MACHINE_SQL =
            "INSERT INTO ${TABLE_PREFIX}state_machine_def ("
                    + STATE_MACHINE_FIELDS
                    + ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    private void init() {
        getGetStateMachineByIdSql =
                GET_STATE_MACHINE_BY_ID_SQL.replaceAll(TABLE_PREFIX_REGEX, tablePrefix);
        queryStateMachinesByNameAndTenantSql =
                QUERY_STATE_MACHINES_BY_NAME_AND_TENANT_SQL.replaceAll(
                        TABLE_PREFIX_REGEX, tablePrefix);
        insertStateMachineSql =
                INSERT_STATE_MACHINE_SQL.replaceAll(TABLE_PREFIX_REGEX, tablePrefix);
    }
