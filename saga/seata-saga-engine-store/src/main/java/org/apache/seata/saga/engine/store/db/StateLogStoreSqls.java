...
public class StateLogStoreSqls {
    ...
    private static final String STATE_MACHINE_INSTANCE_FIELDS =
            "id, machine_id, tenant_id, parent_id, business_key, gmt_started, gmt_end, status,"
                + " compensation_status, is_running, gmt_updated, start_params, end_params, excep";

    private static final String STATE_MACHINE_INSTANCE_FIELDS_WITHOUT_PARAMS =
            "id, machine_id, tenant_id, parent_id, business_key, gmt_started, gmt_end, status,"
                    + " compensation_status, is_running, gmt_updated";

    private static final String RECORD_STATE_MACHINE_STARTED_SQL =
            "INSERT INTO ${TABLE_PREFIX}state_machine_inst\n"
                + "(id, machine_id, tenant_id, parent_id, gmt_started, business_key, start_params,"
                + " is_running, status, gmt_updated)\n"
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    private static final String RECORD_STATE_MACHINE_FINISHED_SQL =
            "UPDATE ${TABLE_PREFIX}state_machine_inst SET gmt_end = ?, excep = ?, end_params ="
                + " ?,status = ?, compensation_status = ?, is_running = ?, gmt_updated = ? WHERE id"
                + " = ? and gmt_updated = ?";
    ...
    private static final String GET_STATE_MACHINE_INSTANCE_BY_BUSINESS_KEY_SQL =
            "SELECT "
                    + STATE_MACHINE_INSTANCE_FIELDS
                    + " FROM ${TABLE_PREFIX}state_machine_inst WHERE business_key = ? AND tenant_id"
                    + " = ?";

    private static final String QUERY_STATE_MACHINE_INSTANCES_BY_PARENT_ID_SQL =
            "SELECT "
                    + STATE_MACHINE_INSTANCE_FIELDS_WITHOUT_PARAMS
                    + " FROM ${TABLE_PREFIX}state_machine_inst WHERE parent_id = ? ORDER BY"
                    + " gmt_started DESC";
    ...
    private static final String STATE_INSTANCE_FIELDS =
            "id, machine_inst_id, name, type, business_key, gmt_started, service_name,"
                + " service_method, service_type, is_for_update, status, input_params,"
                + " output_params, excep, gmt_end, state_id_compensated_for, state_id_retried_for";

    private static final String RECORD_STATE_STARTED_SQL =
            "INSERT INTO ${TABLE_PREFIX}state_inst (id, machine_inst_id, name, type, gmt_started,"
                    + " service_name, service_method, service_type, is_for_update, input_params,"
                    + " status, business_key, state_id_compensated_for, state_id_retried_for,"
                    + " gmt_updated)\n"
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    private static final String RECORD_STATE_FINISHED_SQL =
            "UPDATE ${TABLE_PREFIX}state_inst SET gmt_end = ?, excep = ?, status = ?, output_params"
                    + " = ?, gmt_updated = ? WHERE id = ? AND machine_inst_id = ?";

    private static final String UPDATE_STATE_EXECUTION_STATUS_SQL =
            "UPDATE ${TABLE_PREFIX}state_inst SET status = ?, gmt_updated = ? WHERE machine_inst_id"
                    + " = ? AND id = ?";

    private static final String QUERY_STATE_INSTANCES_BY_MACHINE_INSTANCE_ID_SQL =
            "SELECT "
                    + STATE_INSTANCE_FIELDS
                    + " FROM ${TABLE_PREFIX}state_inst WHERE machine_inst_id = ? ORDER BY"
                    + " gmt_started, ID ASC";
    ...
    private void init() {
        recordStateMachineStartedSql =
                RECORD_STATE_MACHINE_STARTED_SQL.replaceAll(TABLE_PREFIX_REGEX, tablePrefix);
        recordStateMachineFinishedSql =
                RECORD_STATE_MACHINE_FINISHED_SQL.replaceAll(TABLE_PREFIX_REGEX, tablePrefix);
        ...
        getStateMachineInstanceByBusinessKeySql =
                GET_STATE_MACHINE_INSTANCE_BY_BUSINESS_KEY_SQL.replaceAll(
                        TABLE_PREFIX_REGEX, tablePrefix);
        queryStateMachineInstancesByParentIdSql =
                QUERY_STATE_MACHINE_INSTANCES_BY_PARENT_ID_SQL.replaceAll(
                        TABLE_PREFIX_REGEX, tablePrefix);

        recordStateStartedSql =
                RECORD_STATE_STARTED_SQL.replaceAll(TABLE_PREFIX_REGEX, tablePrefix);
        recordStateFinishedSql =
                RECORD_STATE_FINISHED_SQL.replaceAll(TABLE_PREFIX_REGEX, tablePrefix);
        updateStateExecutionStatusSql =
                UPDATE_STATE_EXECUTION_STATUS_SQL.replaceAll(TABLE_PREFIX_REGEX, tablePrefix);
        queryStateInstancesByMachineInstanceIdSql =
                QUERY_STATE_INSTANCES_BY_MACHINE_INSTANCE_ID_SQL.replaceAll(
                        TABLE_PREFIX_REGEX, tablePrefix);
        getStateInstanceByIdAndMachineInstanceIdSql =
                GET_STATE_INSTANCE_BY_ID_AND_MACHINE_INSTANCE_ID_SQL.replaceAll(
                        TABLE_PREFIX_REGEX, tablePrefix);
    }
