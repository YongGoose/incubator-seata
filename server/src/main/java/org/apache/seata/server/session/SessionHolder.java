...
public class SessionHolder {
    ...
    private static long DISTRIBUTED_LOCK_EXPIRE_TIME =
            CONFIG.getLong(
                    ConfigurationKeys.DISTRIBUTED_LOCK_EXPIRE_TIME,
        ...
        if (SessionMode.DB.equals(sessionMode)) {
            ...
            ROOT_VGROUP_MAPPING_MANAGER =
                    EnhancedServiceLoader.load(
                            VGroupMappingStoreManager.class, SessionMode.DB.getName());
            ...
            if (SessionMode.RAFT.equals(sessionMode)) {
                ...
                ROOT_VGROUP_MAPPING_MANAGER =
                        EnhancedServiceLoader.load(
                                VGroupMappingStoreManager.class, SessionMode.RAFT.getName());
            ...
            ROOT_SESSION_MANAGER =
                    EnhancedServiceLoader.load(SessionManager.class, SessionMode.REDIS.getName());
            ROOT_VGROUP_MAPPING_MANAGER =
                    EnhancedServiceLoader.load(
                            VGroupMappingStoreManager.class, SessionMode.REDIS.getName());
            reload(sessionMode);
        } else {
            // unknown store
    ...
    public static void reload(
            Collection<GlobalSession> allSessions, SessionMode storeMode, boolean acquireLock) {
        if ((SessionMode.FILE == storeMode || SessionMode.RAFT == storeMode)
                ...
                switch (globalStatus) {
                    ...
                    case CommitRetrying:
                        if (Objects.equals(SessionMode.RAFT, storeMode)) {
                            // When a state change occurs, re-electing the leader may result in the
                            // lock not being unlocked yet
                            // so a COMMIT unlock operation needs to be performed at the time of
                            // re-election
                            try {
                                globalSession.clean();
                            } catch (TransactionException e) {
                    ...
                    default:
                        {
                            if (acquireLock) {
                                ...
                                if (GlobalStatus.Rollbacking.equals(globalSession.getStatus())
                                        || GlobalStatus.TimeoutRollbacking.equals(
                                                globalSession.getStatus())) {
                                    globalSession.getBranchSessions().parallelStream()
                                            .forEach(
                                                    branchSession ->
                                                            branchSession.setLockStatus(
                            ...
                            switch (globalStatus) {
                                ...
                                case Begin:
                                    if (Objects.equals(storeMode, SessionMode.RAFT)) {
                                        // Avoid rolling back the global session created after
                                        // becoming the leader.
                                        if (globalSession.getBeginTime() < currentTimeMillis) {
                                            try {
                                                globalSession.changeGlobalStatus(
                                ...
                                default:
                                    LOGGER.error(
                                            "Could not handle the global session, xid: {}",
                                            globalSession.getXid());
                                    throw new ShouldNeverHappenException(
                                            "NOT properly handled " + globalStatus);
                            }
                            break;
                        }
                }
            ...
            CompletableFuture.runAsync(
                    () -> {
                        SessionCondition searchCondition =
                        ...
                        while (!CollectionUtils.isEmpty(errorStatusGlobalSessions)) {
                            for (GlobalSession errorStatusGlobalSession :
                                    errorStatusGlobalSessions) {
    ...
    public static boolean acquireDistributedLock(String lockKey) {
        return DISTRIBUTED_LOCKER.acquireLock(
                new DistributedLockDO(
                        lockKey, XID.getIpAddressAndPort(), DISTRIBUTED_LOCK_EXPIRE_TIME));
    }
        ...
        try {
            ...
            if (lock) {
                try {
                    ...
                    LOGGER.warn(
                            "release distribute lock failure, message = {}", ex.getMessage(), ex);
                }
            }
        }
        return lock;
