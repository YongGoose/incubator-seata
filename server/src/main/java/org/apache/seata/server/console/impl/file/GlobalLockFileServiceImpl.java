...
public class GlobalLockFileServiceImpl extends AbstractLockService implements GlobalLockService {
    ...
    public PageResult<GlobalLockVO> query(GlobalLockParam param) {
        ...
        List<GlobalLockQueryBO> result =
                allSessions.parallelStream()
                        .filter(obtainGlobalSessionPredicate(param))
                        .flatMap(
                                globalSession ->
                                        globalSession.getBranchSessions().stream()
    ...
    public SingleResult<Void> deleteLock(GlobalLockParam param) {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    "start to delete global lock,xid:{} branchId:{}",
                    param.getXid(),
                    param.getBranchId());
        }
        GlobalSession globalSession =
                SessionHolder.getRootSessionManager().findGlobalSession(param.getXid(), true);
        if (globalSession != null) {
            ...
            if (branchSessions.size() != 1) {
                throw new ConsoleException(
                        ...
                        String.format(
                                "delete global lock," + "xid:%s ,branchId:%s",
    ...
    private Stream<RowLock> filterAndMap(GlobalLockParam param, BranchSession branchSession) {
        ...
        final List<RowLock> rowLocks =
                LockerManagerFactory.getLockManager().collectRowLocks(branchSession);
    ...
    private Predicate<? super BranchSession> obtainBranchSessionPredicate(GlobalLockParam param) {
        return branchSession -> {
            ...
            return (isBlank(param.getTransactionId())
                            || String.valueOf(branchSession.getTransactionId())
                                    .contains(param.getTransactionId()))
                    &&
                    // branch id
                    (isBlank(param.getBranchId())
                            || String.valueOf(branchSession.getBranchId())
                                    .contains(param.getBranchId()));
        };
    }
        ...
        return globalSession -> {
            ...
            return CollectionUtils.isNotEmpty(globalSession.getBranchSessions())
                    ...
                    (isNull(param.getTimeStart())
                            || param.getTimeStart() / 1000 >= globalSession.getBeginTime() / 1000)
                    &&
                    // timeEnd
                    (isNull(param.getTimeEnd())
                            || param.getTimeEnd() / 1000 <= globalSession.getBeginTime() / 1000);
        };
    }
}
