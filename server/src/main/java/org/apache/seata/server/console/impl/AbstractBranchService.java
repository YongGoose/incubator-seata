...
public abstract class AbstractBranchService extends AbstractService
    ...
    public SingleResult<Void> stopBranchRetry(String xid, String branchId) {
        ...
        if (globalSession.isSaga()) {
            throw new IllegalArgumentException(
                    "saga can not operate branch transactions because it have no determinative"
                            + " role");
        }
        ...
        try {
            ...
            throw new ConsoleException(
                    e,
                    String.format(
                            "stop branch session retry fail, xid:%s, branchId:%s", xid, branchId));
        }
    ...
    public SingleResult<Void> startBranchRetry(String xid, String branchId) {
        ...
        if (!BranchStatus.STOP_RETRY.equals(branchStatus)) {
            throw new IllegalArgumentException(
                    "current branch transactions status is not support to start retry");
        }
        // BranchStatus.PhaseOne_Done and BranchStatus.Registered will become
        ...
        try {
            ...
            throw new ConsoleException(
                    e,
                    String.format(
                            "start branch session retry fail, xid:%s, branchId:%s", xid, branchId));
        }
        return SingleResult.success();
    }
        ...
        if (FAIL_STATUS.contains(globalStatus)
            ...
            try {
                ...
                throw new ConsoleException(
                        e,
                        String.format(
                                "delete branch session fail, xid:%s, branchId:%s", xid, branchId));
            }
        }
        throw new IllegalArgumentException(
                "current global transaction is not support delete branch transaction");
    }
