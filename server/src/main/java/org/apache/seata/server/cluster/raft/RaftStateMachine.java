...
public class RaftStateMachine extends StateMachineAdapter {
    ...
    private static final ScheduledThreadPoolExecutor RESYNC_METADATA_POOL =
            new ScheduledThreadPoolExecutor(
                    1, new NamedThreadFactory("reSyncMetadataPool", 1, true));
    ...
    public void onSnapshotSave(final SnapshotWriter writer, final Closure done) {
        ...
        LOGGER.info(
                "groupId: {}, onSnapshotSave cost: {} ms.",
                group,
    ...
    public boolean onSnapshotLoad(final SnapshotReader reader) {
        ...
        LOGGER.info(
                "groupId: {}, onSnapshotLoad cost: {} ms.",
                group,
                System.currentTimeMillis() - current);
        return true;
    }
    ...
    public void onLeaderStart(final long term) {
        ...
        if (!leader && RaftServerManager.isRaftMode()) {
            ...
            // A member change might trigger a leader re-election. At this point, it’s necessary to
            // filter out non-existent members and synchronize again.
            changePeers(conf);
        }
    }
    ...
    public void onStartFollowing(final LeaderChangeContext ctx) {
        ...
        CompletableFuture.runAsync(
                () -> syncCurrentNodeInfo(ctx.getLeaderId()), RESYNC_METADATA_POOL);
    }
        ...
        if (isLeader()) {
            ...
            try {
                ...
                RaftTaskUtil.createTask(
                        status -> refreshClusterMetadata(raftClusterMetadataMsg),
                        raftClusterMetadataMsg,
                        null);
            } catch (Exception e) {
    ...
    private void onExecuteRaft(RaftBaseMsg msg) {
        ...
        if (execute == null) {
            throw new RuntimeException(
                    "the state machine does not allow events that cannot be executed, please"
                            + " feedback the information to the Seata community !!! msg: "
                            + msg);
        }
        try {
            execute.execute(msg);
            ...
            LOGGER.error(
                    "Message synchronization failure: {}, msgType: {}",
                    e.getMessage(),
                    msg.getMsgType(),
                    e);
    ...
    public RaftClusterMetadata changeOrInitRaftClusterMetadata() {
        ...
        // After the re-election, the leader information may be different from the latest leader,
        // and you need to replace the leader information
        if (leaderNode == null
                || (leaderNode.getInternal() != null
                        && !cureentPeerId.equals(
                                new PeerId(
                                        leaderNode.getInternal().getHost(),
                                        leaderNode.getInternal().getPort())))) {
            Node leader =
                    raftClusterMetadata.createNode(
                            cureentPeerId.getIp(),
                            XID.getPort(),
    ...
    public void refreshClusterMetadata(RaftBaseMsg syncMsg) {
        ...
        ((ApplicationEventPublisher)
                ...
                .publishEvent(
                        new ClusterChangeEvent(
                                this, group, raftClusterMetadata.getTerm(), this.isLeader()));
        ...
        if (initSync.compareAndSet(false, true)) {
            try {
                RouteTable.getInstance()
                        .refreshLeader(
                                RaftServerManager.getCliClientServiceInstance(), group, 1000);
    ...
    private void syncCurrentNodeInfo(PeerId leaderPeerId) {
        try {
            // Ensure that the current leader must be version 2.1 or later to synchronize the
            // operation
            ...
            if (leader != null && StringUtils.isNotBlank(leader.getVersion())) {
                ...
                node.setRole(
                        configuration.getPeers().contains(cureentPeerId)
                                ? ClusterRole.FOLLOWER
                                : ClusterRole.LEARNER);
                invokeContext.put(
                        com.alipay.remoting.InvokeContext.BOLT_CUSTOM_SERIALIZER,
                ...
                // The previous leader may be an old snapshot or log playback, which is not
                // accurate, and you
                                ...
                                (result, err) -> {
                                    if (err == null) {
                                        ...
                                        if (putNodeMetadataResponse.isSuccess()) {
                                            ...
                                            LOGGER.info(
                                                    "sync node info to leader: {}, result: {}",
                                                    leaderPeerId,
                                                    result);
                                        } else {
                                            initSync.compareAndSet(true, false);
                                            LOGGER.info(
                                                    "sync node info to leader: {}, result: {},"
                                                        + " retry will be made at the time of the"
                                                        + " re-election or after 10 seconds",
                                                    leaderPeerId,
                                                    result);
                                        }
                                    } else {
    ...
    public void changeNodeMetadata(Node node) {
        ...
        try {
            List<Node> list =
                    node.getRole() == ClusterRole.FOLLOWER
