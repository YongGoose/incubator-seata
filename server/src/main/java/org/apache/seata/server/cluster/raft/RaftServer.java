...
public class RaftServer implements Disposable, Closeable {
    ...
    public void start() throws IOException {
        ...
        if (reporterEnabled) {
            final Slf4jReporter reporter =
                    Slf4jReporter.forRegistry(node.getNodeMetrics().getMetricRegistry())
                            .outputTo(logger)
            ...
            reporter.start(
                    ConfigurationFactory.getInstance()
                            .getInt(SERVER_RAFT_REPORTER_INITIAL_DELAY, 60),
                    TimeUnit.MINUTES);
        }
    }
    ...
    @Override
    public void destroy() {
        Optional.ofNullable(raftGroupService)
                .ifPresent(
                        r -> {
                            r.shutdown();
                            try {
    ...
    private void enableSSL() {
        ...
        setSystemProperty(
                "bolt.server.ssl.kmf.algorithm", instance.getConfig(SERVER_RAFT_SSL_KMF_ALGORITHM));
        setSystemProperty(
        ...
        setSystemProperty(
                "bolt.client.ssl.tmf.algorithm", instance.getConfig(SERVER_RAFT_SSL_TMF_ALGORITHM));
    ...
    private void setSystemProperty(String property, String value) {
        if (value == null || value.isEmpty()) {
            throw new IllegalArgumentException(
                    "Configuration value for " + property + " cannot be null or empty");
        }
