...
public class RaftServerManager {
    ...
    private static final Map<String /*group*/, RaftServer /*raft-group-cluster*/> RAFT_SERVER_MAP =
            new HashMap<>();
    private static final AtomicBoolean INIT = new AtomicBoolean(false);

    private static final org.apache.seata.config.Configuration CONFIG =
    ...
    public static void init() {
        if (INIT.compareAndSet(false, true)) {
                ...
                if (RAFT_MODE) {
                    for (RegistryService<?> instance : MultiRegistryFactory.getInstances()) {
                        if (!(instance instanceof FileRegistryServiceImpl)
                            ...
                            throw new IllegalArgumentException(
                                    "Raft store mode not support other Registration Center");
                        }
                    }
                }
                ...
                for (PeerId peer : initConf.getPeers()) {
                    ...
                    for (String peerIp : peerIps) {
                        if (StringUtils.equals(peerIp, host)) {
                            if (serverId != null) {
                                throw new IllegalArgumentException(
                                        "server.raft.cluster has duplicate ip, For local debugging,"
                                            + " use -Dserver.raftPort to specify the raft port");
                            }
                            serverId = peer;
                            break;
                        }
            ...
            try {
                // Here you have raft RPC and business RPC using the same RPC server, and you can
                // usually do this
                // separately
                rpcServer = RaftRpcServerFactory.createRaftRpcServer(serverId.getEndpoint());
                RaftServer raftServer =
                        new RaftServer(
                                dataPath, group, serverId, initNodeOptions(initConf), rpcServer);
    ...
    public static void start() {
        ...
        if (rpcServer != null) {
            ...
            SerializerManager.addSerializer(
                    SerializerType.JACKSON.getCode(), new JacksonBoltSerializer());
    ...
    public static boolean isLeader(String group) {
        ...
        Optional.ofNullable(RAFT_SERVER_MAP.get(group))
                .ifPresent(
                        raftServer -> {
    ...
    private static RaftOptions initRaftOptions() {
        ...
        raftOptions.setApplyBatch(
                CONFIG.getInt(SERVER_RAFT_APPLY_BATCH, raftOptions.getApplyBatch()));
        raftOptions.setMaxAppendBufferSize(
                CONFIG.getInt(
                        SERVER_RAFT_MAX_APPEND_BUFFER_SIZE, raftOptions.getMaxAppendBufferSize()));
        raftOptions.setDisruptorBufferSize(
                CONFIG.getInt(
                        SERVER_RAFT_DISRUPTOR_BUFFER_SIZE, raftOptions.getDisruptorBufferSize()));
        raftOptions.setMaxReplicatorInflightMsgs(
                CONFIG.getInt(
                        SERVER_RAFT_MAX_REPLICATOR_INFLIGHT_MSGS,
                        raftOptions.getMaxReplicatorInflightMsgs()));
    ...
    private static NodeOptions initNodeOptions(Configuration initConf) {
        ...
        nodeOptions.setElectionTimeoutMs(
                CONFIG.getInt(
                        SERVER_RAFT_ELECTION_TIMEOUT_MS, DEFAULT_SERVER_RAFT_ELECTION_TIMEOUT_MS));
    ...
    private static class SingletonHandler {
        private static final CliService CLI_SERVICE =
