...
public class LockStoreDataBaseDAO implements LockStore {
    ...
    public boolean acquireLock(List<LockDO> lockDOs, boolean autoCommit, boolean skipCheckLock) {
        ...
        try {
            ...
            if (!skipCheckLock) {
                ...
                // query
                String checkLockSQL =
                        LockStoreSqlFactory.getLogStoreSql(dbType)
                ...
                while (rs.next()) {
                    ...
                    if (!StringUtils.equals(dbXID, currentXID)) {
                        if (LOGGER.isInfoEnabled()) {
                            ...
                            String dbTableName =
                                    rs.getString(ServerTableColumnsName.LOCK_TABLE_TABLE_NAME);
                            long dbBranchId =
                                    rs.getLong(ServerTableColumnsName.LOCK_TABLE_BRANCH_ID);
                            LOGGER.info(
    ...
    public boolean unLock(String xid) {
        ...
        try {
            ...
            String batchDeleteSQL =
                    LockStoreSqlFactory.getLogStoreSql(dbType)
                            .getBatchDeleteLockSqlByXid(lockTable);
            ps = conn.prepareStatement(batchDeleteSQL);
            ps.setString(1, xid);
            ps.executeUpdate();
        } catch (SQLException e) {
    ...
    protected boolean doAcquireLock(Connection conn, LockDO lockDO) {
        ...
        try {
            ...
            String insertLockSQL =
                    LockStoreSqlFactory.getLogStoreSql(dbType).getInsertLockSQL(lockTable);
