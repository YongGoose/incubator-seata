...
public class RaftSnapshotSerializer {
    ...
    static {
        ...
        PERMITS.add(
                io.seata.server.cluster.raft.snapshot.RaftSnapshot.SnapshotType.class.getName());
    ...
    public static byte[] encode(RaftSnapshot raftSnapshot) throws IOException {
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream();
            ...
            Optional.ofNullable(raftSnapshot.getBody())
                    .ifPresent(
                            value ->
                                    raftSnapshot.setBody(
                                            CompressorFactory.getCompressor(
                                                            raftSnapshot.getCompressor())
    ...
    public static byte[] encode(io.seata.server.cluster.raft.snapshot.RaftSnapshot raftSnapshot)
            throws IOException {
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream();
                ObjectOutputStream oos = new ObjectOutputStream(bos)) {
            Serializer serializer =
        ...
        try (ByteArrayInputStream bin = new ByteArrayInputStream(raftSnapshotByte);
            ...
            if (object instanceof io.seata.server.cluster.raft.snapshot.RaftSnapshot) {
                ...
                raftSnapshot.setType(
                        RaftSnapshot.SnapshotType.valueOf(oldRaftSnapshot.getType().name()));
            } else {
