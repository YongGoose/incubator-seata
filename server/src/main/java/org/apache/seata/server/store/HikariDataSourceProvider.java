...
public class HikariDataSourceProvider extends AbstractDataSourceProvider {
    ...
    public DataSource doGenerate() {
        ...
        long idleTimeout =
                CONFIG.getLong(
                        ConfigurationKeys.STORE_DB_HIKARI_IDLE_TIMEOUT,
                        DEFAULT_DB_HIKARI_IDLE_TIMEOUT);
        config.setIdleTimeout(idleTimeout < 0 ? DEFAULT_DB_HIKARI_IDLE_TIMEOUT : idleTimeout);
        long keepaliveTime =
                CONFIG.getLong(
                        ConfigurationKeys.STORE_DB_HIKARI_KEEPALIVE_TIME,
                        DEFAULT_DB_HIKARI_KEEPALIVE_TIME);
        config.setKeepaliveTime(
                keepaliveTime < 0 ? DEFAULT_DB_HIKARI_KEEPALIVE_TIME : keepaliveTime);
        long maxLifeTime =
                CONFIG.getLong(
                        ConfigurationKeys.STORE_DB_HIKARI_MAX_LIFE_TIME,
                        DEFAULT_DB_HIKARI_MAX_LIFE_TIME);
        config.setMaxLifetime(maxLifeTime < 0 ? DEFAULT_DB_HIKARI_MAX_LIFE_TIME : maxLifeTime);
        long validationTimeout =
                CONFIG.getLong(
                        ConfigurationKeys.STORE_DB_HIKARI_VALIDATION_TIMEOUT,
                        DEFAULT_DB_HIKARI_VALIDATION_TIMEOUT);
        config.setValidationTimeout(
                validationTimeout < 0 ? DEFAULT_DB_HIKARI_VALIDATION_TIMEOUT : validationTimeout);
