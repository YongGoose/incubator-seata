...
public abstract class AbstractGlobalService extends AbstractService
        ...
        if (FAIL_STATUS.contains(globalStatus)
            ...
            try {
                ...
                throw new ConsoleException(
                        e, String.format("delete global session fail, xid:%s", xid));
            }
        }
        throw new IllegalArgumentException(
                "current global transaction status is not support deleted");
    }
    ...
    public SingleResult<Void> forceDeleteGlobalSession(String xid) {
        ...
        try {
            ...
            throw new ConsoleException(
                    e, String.format("force delete global session fail, xid:%s", xid));
        }
    }
    ...
    public SingleResult<Void> stopGlobalRetry(String xid) {
        ...
        GlobalStatus newStatus =
                COMMIT_ING_STATUS.contains(globalStatus)
                        ? GlobalStatus.StopCommitOrCommitRetry
                        : RETRY_ROLLBACK_STATUS.contains(globalStatus)
                                        || ROLLBACK_ING_STATUS.contains(globalStatus)
                                ? GlobalStatus.StopRollbackOrRollbackRetry
                                : null;
        if (newStatus == null) {
            throw new IllegalArgumentException(
                    "current global transaction status is not support stop");
        }
        try {
            ...
            throw new ConsoleException(
                    e, String.format("Stop global session retry fail, xid:%s", xid));
        }
    }
    ...
    public SingleResult<Void> startGlobalRetry(String xid) {
        ...
        if (newStatus == null) {
            throw new IllegalArgumentException(
                    "current global transaction status is not support start");
        }
        try {
            globalSession.changeGlobalStatus(newStatus);
            return SingleResult.success();
            ...
            throw new ConsoleException(
                    e, String.format("Start global session retry fail, xid:%s", xid));
        }
    }
    ...
    public SingleResult<Void> sendCommitOrRollback(String xid) {
        ...
        try {
            ...
            if (RETRY_COMMIT_STATUS.contains(globalStatus)
                ...
                throw new IllegalArgumentException(
                        "current global transaction status is not support to do");
            }
            return res
            ...
            throw new ConsoleException(
                    e, String.format("send commit or rollback to rm fail, xid:%s", xid));
        }
    }
    ...
    public SingleResult<Void> changeGlobalStatus(String xid) {
        ...
        throw new IllegalArgumentException(
                "current global transaction status is not support to change");
    }
}
