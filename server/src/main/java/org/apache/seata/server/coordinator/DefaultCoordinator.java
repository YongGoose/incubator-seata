...
public class DefaultCoordinator extends AbstractTCInboundHandler
    ...
    protected static final long COMMITTING_RETRY_PERIOD =
            CONFIG.getLong(
                    ConfigurationKeys.COMMITING_RETRY_PERIOD, DEFAULT_COMMITING_RETRY_PERIOD);
    ...
    protected static final long ASYNC_COMMITTING_RETRY_PERIOD =
            CONFIG.getLong(
                    ConfigurationKeys.ASYNC_COMMITING_RETRY_PERIOD,
                    DEFAULT_ASYNC_COMMITTING_RETRY_PERIOD);
    ...
    protected static final long ROLLBACKING_RETRY_PERIOD =
            CONFIG.getLong(
                    ConfigurationKeys.ROLLBACKING_RETRY_PERIOD, DEFAULT_ROLLBACKING_RETRY_PERIOD);
    ...
    protected static final long END_STATUS_RETRY_PERIOD =
            CONFIG.getLong(
                    ConfigurationKeys.END_STATUS_RETRY_PERIOD, DEFAULT_END_STATUS_RETRY_PERIOD);
    ...
    protected static final long UNDO_LOG_DELETE_PERIOD =
            CONFIG.getLong(
                    ConfigurationKeys.TRANSACTION_UNDO_LOG_DELETE_PERIOD,
                    DEFAULT_UNDO_LOG_DELETE_PERIOD);
     ...
     */
    private static final int BRANCH_ASYNC_POOL_SIZE =
            Runtime.getRuntime().availableProcessors() * 2;

    private static final long MAX_COMMIT_RETRY_TIMEOUT =
            ConfigurationFactory.getInstance()
                    .getLong(
                            ConfigurationKeys.MAX_COMMIT_RETRY_TIMEOUT,
                            DEFAULT_MAX_COMMIT_RETRY_TIMEOUT);

    private static final long MAX_ROLLBACK_RETRY_TIMEOUT =
            ConfigurationFactory.getInstance()
                    .getLong(
    ...
    private static final boolean ROLLBACK_RETRY_TIMEOUT_UNLOCK_ENABLE =
            ConfigurationFactory.getInstance()
                    .getBoolean(
                            ConfigurationKeys.ROLLBACK_RETRY_TIMEOUT_UNLOCK_ENABLE,
                            DEFAULT_ROLLBACK_FAILED_UNLOCK_ENABLE);

    private static final boolean ROLLBACK_FAILED_UNLOCK_ENABLE =
            ConfigurationFactory.getInstance()
                    .getBoolean(
                            ConfigurationKeys.ROLLBACK_FAILED_UNLOCK_ENABLE,
                            DEFAULT_ROLLBACK_FAILED_UNLOCK_ENABLE);
    ...
    private final GlobalStatus[] retryRollbackingStatuses =
            new GlobalStatus[] {
            ...
            branchRemoveExecutor =
                    new ThreadPoolExecutor(
                            ...
                            new ArrayBlockingQueue<>(
                                    CONFIG.getInt(
                                            ConfigurationKeys.SESSION_BRANCH_ASYNC_QUEUE_SIZE,
                                            DEFAULT_BRANCH_ASYNC_QUEUE_SIZE)),
        ...
        if (null == instance) {
            synchronized (DefaultCoordinator.class) {
                if (null == instance) {
                    ...
                    instance =
                            Objects.equals(SessionMode.RAFT, storeMode)
    ...
    protected void doGlobalBegin(
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(
                    "Begin new global transaction applicationId: {},transactionServiceGroup: {},"
                            + " transactionName: {},timeout:{},xid:{}",
                    rpcContext.getApplicationId(),
                    rpcContext.getTransactionServiceGroup(),
                    request.getTransactionName(),
                    request.getTimeout(),
    ...
    protected void timeoutCheck() {
        ...
        if (!beginGlobalSessions.isEmpty() && LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    "Global transaction timeout check begin, size: {}", beginGlobalSessions.size());
        }
        SessionHelper.forEach(
                beginGlobalSessions,
                globalSession -> {
                    ...
                    SessionHolder.lockAndExecute(
                            ...
                            () -> {
                                ...
                                LOGGER.warn(
                                        "Global transaction[{}] is timeout and will be"
                                                + " rollback,transaction begin time:{} and now:{}",
                                        globalSession.getXid(),
                                        DateFormatUtils.format(
                                                globalSession.getBeginTime(), TIME_FORMAT_PATTERN),
                                        DateFormatUtils.format(
                                                System.currentTimeMillis(), TIME_FORMAT_PATTERN));
    ...
    protected void handleRetryCommitting() {
        SessionCondition retryCommittingSessionCondition =
                new SessionCondition(retryCommittingStatuses);
        ...
        SessionHelper.forEach(
                ...
                committingSession -> {
                    try {
                        if (isRetryTimeout(
                                now, MAX_COMMIT_RETRY_TIMEOUT, committingSession.getBeginTime())) {
    ...
    protected void undoLogDelete() {
        ...
        for (Map.Entry<String, Channel> channelEntry : rmChannels.entrySet()) {
            ...
            deleteRequest.setSaveDays(
                    saveDays > 0 ? saveDays : UndoLogDeleteRequest.DEFAULT_SAVE_DAYS);
            try {
                remotingServer.sendAsyncRequest(channelEntry.getValue(), deleteRequest);
            } catch (Exception e) {
                LOGGER.error(
                        "Failed to async delete undo log resourceId = {}, exception: {}",
                        resourceId,
                        e.getMessage());
            }
        }
    ...
    private void rollbackingSchedule(long delay) {
        syncProcessing.schedule(
                () -> {
                    boolean called =
                            SessionHolder.distributedLockAndExecute(
                                    ROLLBACKING, this::handleRollbackingByScheduled);
                    if (!called) {
    ...
    private void committingSchedule(long delay) {
        syncProcessing.schedule(
                () -> {
                    boolean called =
                            SessionHolder.distributedLockAndExecute(
                                    COMMITTING, this::handleCommittingByScheduled);
                    if (!called) {
    ...
    protected void handleEndStatesByScheduled() {
        ...
        long now = System.currentTimeMillis();
        SessionHelper.forEach(
                needDoEndStatusSessions,
                endStatusSession -> {
                    try {
                        if (isRetryTimeout(
                                now, MAX_ROLLBACK_RETRY_TIMEOUT, endStatusSession.getBeginTime())) {
    ...
    private void endSchedule(long delay) {
        syncProcessing.schedule(
                () -> {
                    boolean called =
                            SessionHolder.distributedLockAndExecute(
                                    END, this::handleEndStatesByScheduled);
                    if (!called) {
    ...
    public void init() {
        retryRollbacking.scheduleAtFixedRate(
                () ->
                        SessionHolder.distributedLockAndExecute(
                                RETRY_ROLLBACKING, this::handleRetryRollbacking),
                0,
        ...
        retryCommitting.scheduleAtFixedRate(
                () ->
                        SessionHolder.distributedLockAndExecute(
                                RETRY_COMMITTING, this::handleRetryCommitting),
                0,
        ...
        asyncCommitting.scheduleAtFixedRate(
                () ->
                        SessionHolder.distributedLockAndExecute(
                                ASYNC_COMMITTING, this::handleAsyncCommitting),
                0,
    ...
    public AbstractResultMessage onRequest(AbstractMessage request, RpcContext context) {
        ...
        AbstractTransactionRequestToTC transactionRequest =
                (AbstractTransactionRequestToTC) request;
    ...
    public void destroy() {
        ...
        try {
            retryRollbacking.awaitTermination(
                    TIMED_TASK_SHUTDOWN_MAX_WAIT_MILLS, TimeUnit.MILLISECONDS);
            retryCommitting.awaitTermination(
                    TIMED_TASK_SHUTDOWN_MAX_WAIT_MILLS, TimeUnit.MILLISECONDS);
            asyncCommitting.awaitTermination(
                    TIMED_TASK_SHUTDOWN_MAX_WAIT_MILLS, TimeUnit.MILLISECONDS);
            timeoutCheck.awaitTermination(
                    TIMED_TASK_SHUTDOWN_MAX_WAIT_MILLS, TimeUnit.MILLISECONDS);
            undoLogDelete.awaitTermination(
                    TIMED_TASK_SHUTDOWN_MAX_WAIT_MILLS, TimeUnit.MILLISECONDS);
            if (branchRemoveExecutor != null) {
                branchRemoveExecutor.awaitTermination(
                        TIMED_TASK_SHUTDOWN_MAX_WAIT_MILLS, TimeUnit.MILLISECONDS);
            }
    ...
    static class BranchRemoveTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                LOGGER.error(
                        "Asynchronous delete branchSession error, xid = {}",
                        globalSession.getXid(),
                        unKnowException);
            } finally {
