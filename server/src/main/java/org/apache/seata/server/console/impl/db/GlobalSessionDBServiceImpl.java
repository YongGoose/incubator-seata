...
public class GlobalSessionDBServiceImpl extends AbstractGlobalService
    ...
    public GlobalSessionDBServiceImpl() {
        ...
        if (StringUtils.isBlank(dbType)) {
            throw new IllegalArgumentException(
                    ConfigurationKeys.STORE_DB_TYPE + " should not be blank");
        }
        ...
        if (StringUtils.isBlank(dbDataSource)) {
            throw new IllegalArgumentException(
                    ConfigurationKeys.STORE_DB_DATASOURCE_TYPE + " should not be blank");
        }
    ...
    public PageResult<GlobalSessionVO> query(GlobalSessionParam param) {
        ...
        String querySessionSql =
                PageUtil.pageSql(sourceSql, dbType, param.getPageNum(), param.getPageSize());
        ...
        try {
            ...
            if (param.isWithBranch()) {
                for (GlobalSessionVO globalSessionVO : list) {
                    PageResult<BranchSessionVO> pageResp =
                            branchSessionService.queryByXid(globalSessionVO.getXid());
