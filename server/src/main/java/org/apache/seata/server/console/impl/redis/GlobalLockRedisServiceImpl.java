    ...
    public PageResult<GlobalLockVO> query(GlobalLockParam param) {
        ...
        if (isNotBlank(param.getXid())) {
            ...
            return PageResult.success(
                    globalLockVos, total, param.getPageNum(), param.getPageSize());
        } else if (isNotBlank(param.getTableName())
                && isNotBlank(param.getPk())
                && isNotBlank(param.getResourceId())) {
    ...
    private List<GlobalLockVO> readGlobalLockByXid(String key) {
        ...
        try (Jedis jedis = JedisPooledFactory.getJedisInstance()) {
            ...
            if (CollectionUtils.isNotEmpty(mapGlobalKeys)) {
                ...
                mapGlobalKeys.forEach(
                        (k, v) ->
                                rowLockKeys.addAll(
                                        Arrays.asList(v.split(ROW_LOCK_KEY_SPLIT_CHAR))));
                for (String rowLoclKey : rowLockKeys) {
                    Map<String, String> mapRowLockKey = jedis.hgetAll(rowLoclKey);
                    GlobalLockVO vo =
                            (GlobalLockVO) BeanUtils.mapToObject(mapRowLockKey, GlobalLockVO.class);
                    if (vo != null) {
                        vos.add(vo);
                    }
                }
