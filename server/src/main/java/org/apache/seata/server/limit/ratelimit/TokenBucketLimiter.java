...
public class TokenBucketLimiter implements RateLimiter, Initialize {
    ...
    public TokenBucketLimiter(
            boolean enable,
            Integer bucketTokenNumPerSecond,
            Integer bucketTokenMaxNum,
            Integer bucketTokenInitialNum) {
        this.enable = enable;
    ...
    public void init() {
        ...
        if (this.enable) {
            initBucket();
            LOGGER.info(
                    "TokenBucketLimiter init success, bucketTokenNumPerSecond: {}, tokenMaxNum: {},"
                            + " tokenInitialNum: {}",
                    this.bucketTokenNumPerSecond,
                    this.bucketTokenMaxNum,
                    this.bucketTokenInitialNum);
    ...
    public void reInit(RateLimiterHandlerConfig config) {
        ...
        if (this.enable) {
            initBucket();
            LOGGER.info(
                    "TokenBucketLimiter reInit success, bucketTokenNumPerSecond: {}, tokenMaxNum:"
                            + " {}, tokenInitialNum: {}",
                    this.bucketTokenNumPerSecond,
                    this.bucketTokenMaxNum,
                    this.bucketTokenInitialNum);
    ...
    private void initBucket() {
        Bandwidth limit =
                Bandwidth.classic(
                        this.bucketTokenMaxNum,
                        Refill.greedy(this.bucketTokenNumPerSecond, Duration.ofSeconds(1)));
