...
public class DefaultCoreForEventBusTest {
    ...
    public void test() throws IOException, TransactionException, InterruptedException {
        class GlobalTransactionEventSubscriber {
            ...
            public void processGlobalTransactionEvent(GlobalTransactionEvent event) {
                AtomicInteger counter =
                        eventCounters.computeIfAbsent(
                                event.getStatus(), status -> new AtomicInteger(0));
                counter.addAndGet(1);
        ...
        try {
            ...
            Assertions.assertEquals(
                    1, subscriber.getEventCounters().get(GlobalStatus.Begin.name()).get());
            Assertions.assertEquals(
                    1,
                    subscriber.getEventCounters().get(GlobalStatus.AsyncCommitting.name()).get());
            ...
            subscriber.getDownLatch().await();
            Assertions.assertEquals(
                    2, subscriber.getEventCounters().get(GlobalStatus.Begin.name()).get());
            // Because of the delayed deletion of GlobalSession, and without changing the status of
            // the Session,
            Assertions.assertEquals(
                    1, subscriber.getEventCounters().get(GlobalStatus.Rollbacking.name()).get());
            Assertions.assertNotNull(
                    subscriber.getEventCounters().get(GlobalStatus.Rollbacked.name()));

            // start more one new transaction for test timeout and let this transaction immediately
            // timeout
            ...
            Assertions.assertTrue(
                    subscriber.getEventCounters().get(GlobalStatus.TimeoutRollbacking.name()).get()
                            >= 1);
        } finally {
    ...
    public static void setDown() throws InterruptedException {
        Optional.ofNullable(DefaultCoordinator.getInstance())
                .ifPresent(DefaultCoordinator::destroy);
