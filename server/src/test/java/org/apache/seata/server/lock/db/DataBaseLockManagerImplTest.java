...
public class DataBaseLockManagerImplTest {
    ...
    private static void prepareTable(BasicDataSource dataSource) {
        ...
        try {
            ...
            s.execute(
                    "CREATE TABLE lock_table ( xid varchar(96) , transaction_id long , branch_id"
                        + " long, resource_id varchar(32) ,table_name varchar(32) ,pk varchar(32) "
                        + " ,  row_key  varchar(128) primary key not null , status  integer ,"
    ...
    public void acquireLock() throws TransactionException, SQLException {
        ...
        String sql2 =
                "select count(*) from lock_table where xid = 'abc-123:786756' and row_key in"
                        + " ('abcss^^^t1^^^13', 'abcss^^^t1^^^14', 'abcss^^^t2^^^11',"
                        + " 'abcss^^^t2^^^12')";
    ...
    public void re_acquireLock() throws TransactionException, SQLException {
        ...
        String delSql =
                "delete from lock_table where xid in( 'abc-123:65867978' , 'abc-123:65867978' ,"
                        + " 'abc-123:5678789'  )";
        Connection conn = null;
        Statement stmt = null;
        try {
    ...
    public void unLock() throws TransactionException, SQLException {
        ...
        String sql2 =
                "select count(*) from lock_table where xid = 'abc-123:56867' and row_key in"
                    + " ('abcss^^^t1^^^3', 'abcss^^^t1^^^4', 'abcss^^^t2^^^4', 'abcss^^^t2^^^5')";
        Connection conn = null;
        try {
            conn = dataSource.getConnection();
    ...
    public void isLockable() throws TransactionException, SQLException {
        ...
        Assertions.assertTrue(
                lockManager.isLockable(
                        branchSession2.getXid(),
                        branchSession2.getResourceId(),
        ...
        Assertions.assertFalse(
                lockManager.isLockable(
                        branchSession3.getXid(),
                        branchSession3.getResourceId(),
                        branchSession3.getLockKey()));

        String delSql =
                "delete from lock_table where xid in( 'abc-123:56877898' , 'abc-123:56877898' ,"
                        + " 'abc-123:4575614354'  )";
        Connection conn = null;
        Statement stmt = null;
        try {
