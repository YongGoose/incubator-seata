...
public class ProtobufInnerSerializer {
    ...
    public static byte[] serializeContent(Object request) {
        ...
        Method method =
                CollectionUtils.computeIfAbsent(
                        PROTOBUF_HELPER.toByteArrayMethodMap,
                        clazz,
                        key -> {
                            try {
                                ...
                                throw new ShouldNeverHappenException(
                                        "Cannot found method "
                                                + clazz.getName()
                                                + ".toByteArray(), please check the generated"
                                                + " code.",
                                        e);
                            }
                        });
    ...
    public static <T> T deserializeContent(String responseClazz, byte[] content) {
        ...
        Method method =
                CollectionUtils.computeIfAbsent(
                        PROTOBUF_HELPER.parseFromMethodMap,
                        clazz,
                        key -> {
                            try {
                                ...
                                if (!Modifier.isStatic(m.getModifiers())) {
                                    throw new ShouldNeverHappenException(
                                            "Cannot found static method "
                                                    + clazz.getName()
                                                    + ".parseFrom(byte[]), please check the"
                                                    + " generated code");
                                }
                                m.setAccessible(true);
                                return m;
                                ...
                                throw new ShouldNeverHappenException(
                                        "Cannot found method "
                                                + clazz.getName()
                                                + ".parseFrom(byte[]), please check the generated"
                                                + " code",
                                        e);
                            }
                        });
        ...
        try {
            ...
            throw new ShouldNeverHappenException(
                    "Error when invoke " + clazz.getName() + ".parseFrom(byte[]).", e);
        }
    }
}
