...
public class SqlServerTableMetaCache extends AbstractTableMetaCache {
    ...
    protected String getCacheKey(Connection connection, String tableName, String resourceId) {
        ...
        try {
            ...
            LOGGER.error(
                    "Could not get supportsMixedCaseIdentifiers in connection metadata, use default"
                            + " cache key {}",
                    e.getMessage(),
                    e);
            return cacheKey.append(tableName).toString();
    ...
    private TableMeta resultSetMetaToSchema(Connection connection, String tableName)
        ...
        try (ResultSet rsColumns =
                        metaData.getColumns(catalogName, schemaName, pureTableName, "%");
                ResultSet rsIndex =
                        metaData.getIndexInfo(catalogName, schemaName, pureTableName, false, true);
                ResultSet rsTable =
                        metaData.getTables(
                                catalogName, schemaName, pureTableName, new String[] {"TABLE"});
                ResultSet rsPrimary =
                        metaData.getPrimaryKeys(catalogName, schemaName, pureTableName)) {
            // get column metaData
            while (rsColumns.next()) {
                ...
                if (tm.getAllColumns().containsKey(col.getColumnName())) {
                    throw new NotSupportYetException(
                            "Not support the table has the same column name with different case"
                                    + " yet");
                }
            ...
            while (rsTable.next()) {
                ...
                if ("dbo".equalsIgnoreCase(rsTableSchema)) {
                    // for compatibility reasons, old clients generally do not have the 'dbo'
                    // default schema by default.
                    tm.setTableName(rsTableName);
                } else {
                    // without schema, different records with the same primary key value and the
                    // same table name in different schemas may have the same lock record.
