...
public class MySQLInsertOnDuplicateUpdateExecutor extends MySQLInsertExecutor
    ...
    protected void buildUndoItemAll(
        ...
        if (!isUpdateFlag) {
            SQLUndoLog sqlUndoLog =
                    buildUndoItem(SQLType.INSERT, TableRecords.empty(getTableMeta()), afterImage);
        ...
        if (CollectionUtils.isNotEmpty(updateRows)) {
            ...
            connectionProxy.appendUndoLog(
                    buildUndoItem(SQLType.UPDATE, beforeImage, partAfterImage));
        }
        if (CollectionUtils.isNotEmpty(insertRows)) {
            ...
            connectionProxy.appendUndoLog(
                    buildUndoItem(
                            SQLType.INSERT, TableRecords.empty(getTableMeta()), partAfterImage));
        }
    }
    ...
    protected SQLUndoLog buildUndoItem(
            SQLType sqlType, TableRecords beforeImage, TableRecords afterImage) {
    ...
    protected TableRecords afterImage(TableRecords beforeImage) throws SQLException {
                    ...
                    fields.forEach(
                            f -> {
                                ArrayList<Object> values =
                                        primaryValueMap.computeIfAbsent(
                                                f.getName(), v -> new ArrayList<>());
        ...
        return buildTableRecords2(
                tableMeta, afterImageSql.toString(), paramAppenderList, primaryValues);
    }
    ...
    public TableRecords buildTableRecords2(
        ...
        if (CollectionUtils.isEmpty(paramAppenderList)) {
            throw new NotSupportYetException(
                    "the SQL statement has no primary key or unique index value, it will not hit"
                            + " any row data.recommend to convert to a normal insert statement");
        }
        ...
        try {
            ps =
                    statementProxy
                            .getConnection()
                            .prepareStatement(
                                    primaryKeys.isEmpty() ? selectSQL + " FOR UPDATE" : selectSQL);
        ...
        for (int i = 0; i < insertNum; i++) {
            ...
            tableMeta
                    ...
                    .forEach(
                            (k, v) -> {
                                if (!v.isNonUnique()
                                        && isIndexValueNotNull(v, imageParameterMap, finalI)) {
                                    boolean columnIsNull = true;
                                    List<String> uniqueList = new ArrayList<>();
                                    for (ColumnMeta m : v.getValues()) {
                                        String columnName = m.getColumnName();
                                        List<Object> imageParameters =
                                                imageParameterMap.get(columnName);
    ...
    public Map<String, ArrayList<Object>> buildImageParameters(SQLInsertRecognizer recognizer) {
        ...
        if (CollectionUtils.isNotEmpty(duplicateKeyUpdateColumns)) {
            List<String> duplicateKeyUpdateLowerCaseColumns =
                    duplicateKeyUpdateColumns.parallelStream()
                                ...
                                if ("PRIMARY".equalsIgnoreCase(k)) {
                                    for (ColumnMeta m : v.getValues()) {
                                        if (duplicateKeyUpdateLowerCaseColumns.contains(
                                            ...
                                            throw new ShouldNeverHappenException(
                                                    "update pk value is not supported!");
                                        }
                                    }
                                }
                            });
        ...
        Map<Integer, ArrayList<Object>> parameters =
                ((PreparedStatementProxy) statementProxy).getParameters();
        ...
        for (List<Object> row : insertRows) {
            ...
            for (int i = 0; i < insertColumns.size(); i++) {
                ...
                ArrayList<Object> columnImages =
                        imageParameterMap.computeIfAbsent(column, k -> new ArrayList<>());
