...
public class MysqlTableMetaCache extends AbstractTableMetaCache {
    ...
    protected String getCacheKey(Connection connection, String tableName, String resourceId) {
        ...
        // now: Use the original table name to avoid cache errors of tables with the same name
        // across databases
    ...
    protected TableMeta fetchSchema(Connection connection, String tableName) throws SQLException {
        String sql =
                "SELECT * FROM "
                        + ColumnUtils.addEscape(tableName, JdbcConstants.MYSQL)
                        + " LIMIT 1";
    ...
    protected TableMeta resultSetMetaToSchema(
            ResultSetMetaData rsmd, DatabaseMetaData dbmd, String originalTableName)
            throws SQLException {
        // always "" for mysql
        ...
        try (ResultSet rsColumns = dbmd.getColumns(catalogName, schemaName, tableName, "%");
                ResultSet rsIndex =
                        dbmd.getIndexInfo(catalogName, schemaName, tableName, false, true);
                ResultSet onUpdateColumns =
                        dbmd.getVersionColumns(catalogName, schemaName, tableName)) {
            ...
            if (tm.getAllIndexes().isEmpty()) {
                throw new ShouldNeverHappenException(
                        "Could not found any index in the table: " + tableName);
            }
        }
        return tm;
    }
