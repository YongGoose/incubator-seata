...
public class ConnectionProxyXA extends AbstractConnectionProxyXA implements Holdable {
    ...
    private static final int BRANCH_EXECUTION_TIMEOUT =
            ConfigurationFactory.getInstance()
                    .getInt(
                            XA_BRANCH_EXECUTION_TIMEOUT,
        ...
        if (autoCommit) {
            ...
            if (xaActive) {
                throw new SQLException(
                        "should NEVER happen: setAutoCommit from true to false while xa branch is"
                                + " active");
            }
            // Start a XA branch
            long branchId;
            ...
            try {
                ...
                cleanXABranchContext();
                throw new SQLException(
                        "failed to start xa branch " + xid + " since " + e.getMessage(), e);
            }
            // 4. XA is active
            this.xaActive = true;
        }
    ...
    public void commit() throws SQLException {
        try (ResourceLock ignored = resourceLock.obtain()) {
            ...
            if (!xaActive || this.xaBranchXid == null) {
                throw new SQLException(
                        "should NOT commit on an inactive session", SQLSTATE_XA_NOT_END);
            }
        }
    }
    ...
    public void rollback() throws SQLException {
        ...
        try {
            ...
            throw new SQLException(
                    "Failed to end(TMFAIL) xa branch on "
                            + xid
                            + "-"
    ...
    private void start() throws XAException, SQLException {
        try (ResourceLock ignored = resourceLock.obtain()) {
            ...
            try {
                ...
                // the framework layer does not actively call ROLLBACK when setAutoCommit throws an
                // SQL exception
    ...
    public void close() throws SQLException {
        try (ResourceLock ignored = resourceLock.obtain()) {
            try {
                if (xaActive && this.xaBranchXid != null) {
                    ...
                    try {
                        ...
                        throw new SQLException(
                                "Branch "
                                        + xaBranchXid
                                        + " was rollbacked on committing since "
                                        + sqle.getMessage(),
                                SQLSTATE_XA_NOT_END,
                                sqle);
                    }
                ...
                reportStatusToTC(BranchStatus.PhaseOne_Failed);
                throw new SQLException(
                        "Failed to end(TMSUCCESS)/prepare xa branch on "
                                + xid
                                + "-"
                                + xaBranchXid.getBranchId()
                                + " since "
    ...
    private void termination(String xaBranchXid) throws SQLException {
        ...
        if (branchStatus != null) {
            ...
            throw new SQLException(
                    "failed xa branch "
                            + xid
                            + " the global transaction has finish, branch status: "
    ...
    private void reportStatusToTC(BranchStatus status) {
        try {
            DefaultResourceManager.get()
                    .branchReport(BranchType.XA, xid, xaBranchXid.getBranchId(), status, null);
        } catch (TransactionException te) {
            LOGGER.warn(
