...
public class UndoLogParserFactory {
    ...
    private static class SingletonHolder {
        private static final UndoLogParser INSTANCE =
                getInstance(UndoLogConstants.DEFAULT_SERIALIZER);
    }
    ...
    public static UndoLogParser getInstance(String name) {
        if (StringUtils.equalsIgnoreCase("fst", name)) {
            throw new IllegalArgumentException(
                    "Since fst is no longer maintained, this serialization extension has been"
                            + " removed from version 2.0 for security and stability reasons.");
        }
