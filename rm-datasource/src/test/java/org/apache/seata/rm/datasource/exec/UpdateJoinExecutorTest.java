...
public class UpdateJoinExecutorTest {
    ...
    public void testUpdateJoinUndoLog() throws SQLException {
        ...
        StatementProxy beforeMockStatementProxy =
                mockStatementProxy(
                        returnValueColumnLabels, beforeReturnValue, columnMetas, indexMetas);
        String sql =
                "update t1 inner join t2 on t1.id = t2.id set t1.name = 'WILL',t2.name = 'WILL'";
        List<SQLStatement> asts = SQLUtils.parseStatements(sql, JdbcConstants.MYSQL);
        MySQLUpdateRecognizer recognizer = new MySQLUpdateRecognizer(sql, asts.get(0));
        UpdateExecutor mySQLUpdateJoinExecutor =
                new MySQLUpdateJoinExecutor(
        ...
        StatementProxy afterMockStatementProxy =
                mockStatementProxy(
                        returnValueColumnLabels, afterReturnValue, columnMetas, indexMetas);
        ...
        Assertions.assertDoesNotThrow(
                () -> mySQLUpdateJoinExecutor.prepareUndoLog(beforeImage, afterImage));
    }
