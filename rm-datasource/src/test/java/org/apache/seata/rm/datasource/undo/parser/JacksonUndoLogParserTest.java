...
public class JacksonUndoLogParserTest extends BaseUndoLogParserTest {
    ...
    JacksonUndoLogParser parser =
            (JacksonUndoLogParser)
                    EnhancedServiceLoader.load(UndoLogParser.class, JacksonUndoLogParser.NAME);
    ...
    public void encode()
        ...
        Field field =
                new Field(
                        "bigint_type", JDBCType.BIGINT.getVendorTypeNumber(), 9223372036854775807L);
        ...
        field =
                new Field(
                        "clob_type",
                        JDBCType.CLOB.getVendorTypeNumber(),
                        new SerialClob("hello".toCharArray()));
        bytes = mapper.writeValueAsBytes(field);
        sameField = mapper.readValue(bytes, Field.class);
        Assertions.assertTrue(DataCompareUtils.isFieldEquals(field, sameField).getResult());
    ...
    public void testSerializeAndDeserializeDmdbTimestamp()
        ...
        Object dmdbTimestamp =
                valueOfMethod.invoke(
                        null, Instant.ofEpochMilli(1721985847000L).atZone(ZoneId.systemDefault()));
        Field field =
                new Field(
                        "dmdb_timestamp", JDBCType.TIMESTAMP.getVendorTypeNumber(), dmdbTimestamp);
        byte[] bytes = mapper.writeValueAsBytes(field);
        Field sameField = mapper.readValue(bytes, Field.class);
        Assertions.assertTrue(DataCompareUtils.isFieldEquals(field, sameField).getResult());
        ...
        Object dmdbTimestampnanos =
                valueOfMethod.invoke(
                        null,
                        Instant.ofEpochMilli(1721985847000L)
                                .plusNanos(12345L)
                                .atZone(ZoneId.systemDefault()));
        field =
                new Field(
                        "dmdb_timestamp_nanos",
                        JDBCType.TIMESTAMP.getVendorTypeNumber(),
                        dmdbTimestampnanos);
        bytes = mapper.writeValueAsBytes(field);
        sameField = mapper.readValue(bytes, Field.class);
