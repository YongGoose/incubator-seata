...
public class MySQLInsertExecutorTest {
    ...
    public void init() throws SQLException {
        ...
        resultSet.mockResultSet(
                Arrays.asList("Variable_name", "Value"),
                new Object[][] {{"auto_increment_increment", "1"}});
        when(statementProxy
                        .getTargetStatement()
        ...
        List<String> returnValueColumnLabels =
                Lists.newArrayList("id", "user_id", "name", "sex", "update_time");
        Object[][] returnValue =
                new Object[][] {
    ...
    public void testBeforeAndAfterImage() throws SQLException {
        ...
        String sql =
                "insert into table_insert_executor_test(id, user_id, name, sex) values (1, 1,"
                        + " 'will', 1)";
        List<SQLStatement> asts = SQLUtils.parseStatements(sql, JdbcConstants.MYSQL);
        MySQLInsertRecognizer recognizer = new MySQLInsertRecognizer(sql, asts.get(0));
        newInsertExecutor =
                new MySQLInsertExecutor(newStatementProxy, (statement, args) -> null, recognizer);
    ...
    public void testBeforeAndAfterImageTableSchemaAndTableName() throws SQLException {
        String sql =
                "insert into seata.table_insert_executor_test(id, user_id, name, sex) values (1, 1,"
                        + " 'will', 1)";
        List<SQLStatement> asts = SQLUtils.parseStatements(sql, JdbcConstants.MYSQL);
        MySQLInsertRecognizer recognizer = new MySQLInsertRecognizer(sql, asts.get(0));
        newInsertExecutor =
    ...
    public void testBeforeAndAfterImageTableSchemaWithQuoteAndTableName() throws SQLException {
        String sql =
                "insert into `seata`.table_insert_executor_test(id, user_id, name, sex) values (1,"
                        + " 1, 'will', 1)";
        List<SQLStatement> asts = SQLUtils.parseStatements(sql, JdbcConstants.MYSQL);
        MySQLInsertRecognizer recognizer = new MySQLInsertRecognizer(sql, asts.get(0));
        newInsertExecutor =
    ...
    @Test
    public void testBeforeAndAfterImageTableSchemaWithQuoteAndTableNameWithQuote()
            throws SQLException {
        String sql =
                "insert into `seata`.`table_insert_executor_test`(id, user_id, name, sex) values"
                        + " (1, 1, 'will', 1)";
        List<SQLStatement> asts = SQLUtils.parseStatements(sql, JdbcConstants.MYSQL);
        MySQLInsertRecognizer recognizer = new MySQLInsertRecognizer(sql, asts.get(0));
        newInsertExecutor =
    ...
    public void testBeforeAndAfterImageColumnWithQuote() throws SQLException {
        String sql =
                "insert into table_insert_executor_test(`id`, `user_id`, `name`, `sex`) values (1,"
                        + " 1, 'will', 1)";
        List<SQLStatement> asts = SQLUtils.parseStatements(sql, JdbcConstants.MYSQL);
        MySQLInsertRecognizer recognizer = new MySQLInsertRecognizer(sql, asts.get(0));
        newInsertExecutor =
        ...
        Assertions.assertThrows(
                ...
                () -> {
                    ...
                    when(tableMeta.getPrimaryKeyOnlyName())
                            .thenReturn(Arrays.asList(new String[] {ID_COLUMN}));
    ...
    public void testGetPkValuesByColumn_PkValue_Null() throws SQLException {
        ...
        when(tableMeta.getPrimaryKeyMap())
                .thenReturn(
                        new HashMap<String, ColumnMeta>() {
                            {
                                put(ID_COLUMN, cm);
                            }
                        });
    ...
    public void testGetPkValuesByAuto_ExecuteQuery_HasResult() throws SQLException {
        ...
        PreparedStatement preparedStatement = mock(PreparedStatement.class);
        when(statementProxy.getTargetStatement()).thenReturn(preparedStatement);
        when(statementProxy.getGeneratedKeys())
                .thenThrow(new SQLException("", MySQLInsertExecutor.ERR_SQL_STATE));
        ResultSet resultSet = mock(ResultSet.class);
    ...
    public void test_autoGeneratePks()
        ...
        Method method =
                MySQLInsertExecutor.class.getDeclaredMethod(
                        "autoGeneratePks",
                        new Class[] {BigDecimal.class, String.class, Integer.class});
