...
public class HttpClientUtil {
    ...
    private static final Map<Integer /*timeout*/, CloseableHttpClient> HTTP_CLIENT_MAP =
                                ...
                                () ->
                                        HTTP_CLIENT_MAP.values().parallelStream()
                                                .forEach(
                                                        client -> {
                                                            try {
                                                                // delay 3s, make sure unregister
                                                                // http request send successfully
                                                                Thread.sleep(3000);
                                                                client.close();
                                                            } catch (IOException
    ...
    public static CloseableHttpResponse doPost(
            String url, Map<String, String> params, Map<String, String> header, int timeout)
            throws IOException {
        try {
            ...
            if (StringUtils.isNotBlank(contentType)) {
                if (ContentType.APPLICATION_FORM_URLENCODED.getMimeType().equals(contentType)) {
                    ...
                    StringEntity stringEntity =
                            new StringEntity(requestBody, ContentType.APPLICATION_FORM_URLENCODED);
            ...
            CloseableHttpClient client =
                    HTTP_CLIENT_MAP.computeIfAbsent(
                            timeout,
                            k ->
                                    HttpClients.custom()
    ...
    // post request
    public static CloseableHttpResponse doPost(
            String url, String body, Map<String, String> header, int timeout) throws IOException {
        try {
            URIBuilder builder = new URIBuilder(url);
            URI uri = builder.build();
            HttpPost httpPost = new HttpPost(uri);
    ...
    public static CloseableHttpResponse doGet(
            String url, Map<String, String> param, Map<String, String> header, int timeout)
            throws IOException {
        try {
            URIBuilder builder = new URIBuilder(url);
            if (param != null) {
