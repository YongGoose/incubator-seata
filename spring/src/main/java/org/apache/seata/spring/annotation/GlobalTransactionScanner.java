...
public class GlobalTransactionScanner extends AbstractAutoProxyCreator
    ...
    public GlobalTransactionScanner(
            String applicationId, String txServiceGroup, FailureHandler failureHandlerHook) {
    ...
    protected void initClient() {
        ...
        if (DEFAULT_TX_GROUP_OLD.equals(txServiceGroup)) {
            LOGGER.warn(
                    "the default value of seata.tx-service-group: {} has already changed to {}"
                        + " since Seata 1.5, please change your default configuration as soon as"
                        + " possible and we don't recommend you to use default tx-service-group's"
                        + " value provided by seata",
        ...
        ShutdownHook.getInstance()
                .addDisposable(
                        TmNettyRemotingClient.getInstance(
                                applicationId, txServiceGroup, accessKey, secretKey));
        ShutdownHook.getInstance()
                .addDisposable(RmNettyRemotingClient.getInstance(applicationId, txServiceGroup));
    }
            ...
            synchronized (PROXYED_SET) {
                ...
                if (!AopUtils.isAopProxy(bean)) {
                    ...
                    for (Advisor avr : advisor) {
                        // Find the position based on the advisor's order, and add to advisors by
                        // pos
    ...
    private int findAddSeataAdvisorPosition(AdvisedSupport advised, Advisor seataAdvisor) {
        ...
        SeataInterceptorPosition seataInterceptorPosition =
                getSeataInterceptorPosition(seataAdvisor);
        ...
        if (SeataInterceptorPosition.Any == seataInterceptorPosition) {
            ...
            // If the interceptorPosition is not any, compute position if has
            // TransactionInterceptor.
            Integer position =
                    computePositionIfHasTransactionInterceptor(
    ...
    private Integer computePositionIfHasTransactionInterceptor(
        ...
        if (SeataInterceptorPosition.AfterTransaction == seataInterceptorPosition
            ...
            if (LOGGER.isWarnEnabled()) {
                LOGGER.warn(
                        "The {}'s order '{}' is higher or equals than {}'s order '{}' , reset {}'s"
                                + " order to lower order '{}'.",
            ...
            if (LOGGER.isWarnEnabled()) {
                LOGGER.warn(
                        "The {}'s order '{}' is lower or equals than {}'s order '{}' , reset {}'s"
        ...
        if (applicationContext instanceof ConfigurableApplicationContext) {
            ...
            for (String contextBeanName : beanNames) {
                ...
                try {
                    ...
                    if (ifNeedEnhanceBean
                            .getNeedEnhanceEnum()
                            .equals(NeedEnhanceEnum.SERVICE_BEAN)) {
            ...
            LOGGER.info(
                    "The needed enhancement business beans are : {}", NEED_ENHANCE_BEAN_NAME_SET);
        }
    }
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        ...
        RemotingFactoryBeanParser remotingFactoryBeanParser =
                new RemotingFactoryBeanParser(applicationContext);
