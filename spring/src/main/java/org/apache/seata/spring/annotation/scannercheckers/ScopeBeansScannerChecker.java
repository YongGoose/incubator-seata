...
public class ScopeBeansScannerChecker implements ScannerChecker {
    ...
    public boolean check(
            Object bean, String beanName, @Nullable ConfigurableListableBeanFactory beanFactory)
            throws Exception {
        if (beanFactory == null) {
        ...
        if (beanDefinition != null) {
            ...
            if (annotatedBeanDefinition.getFactoryMethodMetadata() != null) {
                if (this.hasExcludeScope(
                        beanName, annotatedBeanDefinition.getFactoryMethodMetadata())) {
                    ...
                    if (LOGGER.isInfoEnabled()) {
                        LOGGER.info(
                                "Exclude bean `{}` from the `{}`, cause of `@Scope(scopeName ="
                                    ...
                                    + " Please refactor the code if you want to continue using it.",
                                beanName,
