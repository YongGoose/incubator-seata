...
public class ZookeeperRegisterServiceImplTest {
    ...
    ZookeeperRegisterServiceImpl service =
            (ZookeeperRegisterServiceImpl) new ZookeeperRegistryProvider().provide();
    ...
    public void testAll() throws Exception {
        ...
        CuratorCacheListener listener =
                (CuratorCacheListener.Type type, ChildData oldData, ChildData newdata) -> {
                    List<String> list;
                    try {
        ...
        CuratorCacheListener listener2 =
                (CuratorCacheListener.Type type, ChildData oldData, ChildData newdata) -> {
                    List<String> list;
                    try {
                        list = service.getZkClient().getChildren().forPath(newdata.getPath());
    ...
    public void testLookUp() throws Exception {
        ...
        client.create()
                ...
                .forPath("/registry/zk/cluster/127.0.0.1:8091");
        ...
        Assertions.assertEquals(
                addressList, Collections.singletonList(new InetSocketAddress("127.0.0.1", 8091)));
    }

    @Test
    public void testRemoveOfflineAddressesIfNecessaryNoRemoveCase() {
        Map<String, List<InetSocketAddress>> addresses =
                service.CURRENT_ADDRESS_MAP.computeIfAbsent(
                        "default_tx_group", k -> new HashMap<>());
        addresses.put(
                "cluster", Collections.singletonList(new InetSocketAddress("127.0.0.1", 8091)));
        ...
        Assertions.assertEquals(
                1, service.CURRENT_ADDRESS_MAP.get("default_tx_group").get("cluster").size());
    }
    ...
    public void testAliveLookup() {
        ...
        Assertions.assertEquals(
                result, Collections.singletonList(new InetSocketAddress("127.0.0.1", 8091)));
    }
