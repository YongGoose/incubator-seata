...
public class NamingserverRegistryServiceImpl implements RegistryService<NamingListener> {
    ...
    private static final ConcurrentMap<
                    String /* namingserver address */,
                    AtomicInteger /* Number of Health Check Continues Failures */>
            AVAILABLE_NAMINGSERVER_MAP = new ConcurrentHashMap<>();
    private static final ConcurrentMap<String /* vgroup */, List<NamingServerNode>>
            VGROUP_ADDRESS_MAP = new ConcurrentHashMap<>();
    private static final ConcurrentMap<String /* vgroup */, List<NamingListener>>
    ...
    static {
        TOKEN_EXPIRE_TIME_IN_MILLISECONDS =
                FILE_CONFIG.getLong(getTokenExpireTimeInMillisecondsKey(), 29 * 60 * 1000L);
    ...
    private NamingserverRegistryServiceImpl() {
        ...
        String heartBeatKey =
                String.join(
                        FILE_CONFIG_SPLIT_CHAR, FILE_ROOT_REGISTRY, REGISTRY_TYPE, HEART_BEAT_KEY);
    ...
    private void checkAvailableNamingAddr(List<String> urlList) {
        for (String url : urlList) {
            ...
            if (!Objects.equals(unHealthCountAfter, 0)
                    && unHealthCountAfter == HEALTH_CHECK_THRESHOLD) {
    ...
    public void doRegister(Instance instance, List<String> urlList) throws RetryableException {
        for (String urlSuffix : urlList) {
            ...
            String params =
                    "namespace=" + namespace + "&clusterName=" + clusterName + "&unit=" + unit;
            url += params;
            ...
            try (CloseableHttpResponse response =
                    HttpClientUtil.doPost(url, jsonBody, header, 3000)) {
                    ...
                    while (isSubscribed) {
                        try {
                            long metadataMaxAgeMs =
                                    FILE_CONFIG.getLong(getMetadataMaxAgeMs(), 30000L);
                            // pull
                            boolean needFetch =
                                    System.currentTimeMillis() - currentTime > metadataMaxAgeMs;
                            if (!needFetch) {
                                // push
    ...
    public boolean watch(String vGroup) throws RetryableException {
        ...
        StringBuilder watchAddrBuilder =
                new StringBuilder(HTTP_PREFIX)
                        .append(namingAddr)
        ...
        try (CloseableHttpResponse response =
                HttpClientUtil.doPost(watchAddr, (String) null, header, 30000)) {
            if (response != null) {
    ...
    public List<InetSocketAddress> lookup(String key) throws Exception {
        ...
        return Optional.ofNullable(VGROUP_ADDRESS_MAP.get(key))
    ...
    public List<InetSocketAddress> refreshGroup(String vGroup)
        ...
        try (CloseableHttpResponse response = HttpClientUtil.doGet(url, paraMap, header, 3000)) {
            if (response == null || response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                throw new NamingRegistryException(
                        "cannot lookup server list in vgroup: "
                                + vGroup
                                + ", http code: "
            ...
            MetaResponse metaResponse =
                    OBJECT_MAPPER.readValue(jsonResponse, new TypeReference<MetaResponse>() {});
    ...
    public List<InetSocketAddress> handleMetadata(MetaResponse metaResponse, String vGroup) {
        ...
        for (Cluster cluster : metaResponse.getClusterList()) {
            for (Unit unitDatum : cluster.getUnitData()) {
                // In raft mode, only the leader is cached, while in non-raft cluster mode, all
                // nodes are cached.
                newAddressList.addAll(
                        unitDatum.getNamingInstanceList().stream()
                                .filter(
                                        instance ->
                                                (instance.getRole() == ClusterRole.LEADER
                                                                && instance.getTerm() >= term)
    ...
    public String getNamespace() {
        String namespaceKey =
                String.join(
                        FILE_CONFIG_SPLIT_CHAR, FILE_ROOT_REGISTRY, REGISTRY_TYPE, NAMESPACE_KEY);
    ...
    public List<InetSocketAddress> aliveLookup(String transactionServiceGroup) {
        Map<String, List<InetSocketAddress>> clusterAddressMap =
                CURRENT_ADDRESS_MAP.computeIfAbsent(
                        transactionServiceGroup, k -> new ConcurrentHashMap<>());
    ...
    public List<InetSocketAddress> refreshAliveLookup(
        ...
        Map<String, List<InetSocketAddress>> clusterAddressMap =
                CURRENT_ADDRESS_MAP.computeIfAbsent(
                        transactionServiceGroup, key -> new ConcurrentHashMap<>());
    ...
    public String getNamingAddr() {
        ...
        Map<String, AtomicInteger> availableNamingserverMap =
                new HashMap<>(AVAILABLE_NAMINGSERVER_MAP);
    ...
    private static void refreshToken(String tcAddress) throws RetryableException {
        ...
        try (CloseableHttpResponse httpResponse =
                HttpClientUtil.doPost(
                        "http://" + tcAddress + "/api/v1/auth/login", param, header, 1000)) {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (!StringUtils.equals(codeStatus, "200")) {
                        ...
                        throw new AuthenticationFailedException(
                                "Authentication failed! you should configure the correct username"
                                        + " and password.");
                    }
                    ...
                    // authorized failed,throw exception to kill process
                    throw new AuthenticationFailedException(
                            "Authentication failed! you should configure the correct username and"
                                    + " password.");
                }
            }
        } catch (IOException e) {
