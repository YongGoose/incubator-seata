...
public class NacosRegistryServiceImpl implements RegistryService<EventListener> {
    ...
    public void unsubscribe(String cluster, EventListener listener) throws Exception {
        ...
        if (subscribeList != null) {
            List<EventListener> newSubscribeList =
    ...
    public List<InetSocketAddress> lookup(String key) throws Exception {
        ...
        if (useSLBWay) {
            ...
            if (!CLUSTER_ADDRESS_MAP.containsKey(PUBLIC_NAMING_ADDRESS_PREFIX + clusterName)) {
                Service service =
                        getNamingMaintainInstance().queryService(DEFAULT_APPLICATION, clusterName);
                ...
                InetSocketAddress publicAddress =
                        new InetSocketAddress(pubnetIp, Integer.valueOf(pubnetPort));
                List<InetSocketAddress> publicAddressList = Arrays.asList(publicAddress);
                CLUSTER_ADDRESS_MAP.put(
                        PUBLIC_NAMING_ADDRESS_PREFIX + clusterName, publicAddressList);
                ...
                if (!LISTENER_SERVICE_MAP.containsKey(clusterName)) {
                    ...
                    if (null != firstAllInstances) {
                        List<InetSocketAddress> newAddressList =
                                firstAllInstances.stream()
                                        .filter(
                                                eachInstance ->
                                                        eachInstance.isEnabled()
                                                                && eachInstance.isHealthy())
                                        .map(
                                                eachInstance ->
                                                        new InetSocketAddress(
                                                                eachInstance.getIp(),
                                                                eachInstance.getPort()))
                                        .collect(Collectors.toList());
                    ...
                    subscribe(
                            ...
                            event -> {
                                ...
                                if (CollectionUtils.isEmpty(instances)
                                        && null != CLUSTER_ADDRESS_MAP.get(clusterName)) {
                                    LOGGER.info(
                                            "receive empty server list,cluster:{}", clusterName);
                                } else {
                                    List<InetSocketAddress> newAddressList =
                                            instances.stream()
                                                    .filter(
    ...
    private static boolean initNacosAuthProperties(Properties sourceProperties) {
        String userName =
                StringUtils.isNotBlank(System.getProperty(USER_NAME))
