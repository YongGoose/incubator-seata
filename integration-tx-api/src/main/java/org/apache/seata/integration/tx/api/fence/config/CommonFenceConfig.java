    ...
    public void initCleanTask() {
        try {
            ...
            if (cleanPeriod.isZero() || cleanPeriod.isNegative()) {
                LOGGER.info(
                        "Common fence log clean task is not started, cleanPeriod is:{}",
                        cleanPeriod);
                return;
            }
            // convert to second level. maximum interval is 68 years
            long periodSeconds =
                    cleanPeriod.compareTo(MAX_PERIOD) >= 0
                            ? Integer.MAX_VALUE
                            : cleanPeriod.toMillis() / 1000;
                    ...
                    () -> {
                        ...
                        try {
                            ...
                            if (deletedRowCount > 0) {
                                LOGGER.info(
                                        "Common fence clean task executed success, timeBefore: {},"
                                                + " deleted row count: {}",
                                        timeBefore,
                                        deletedRowCount);
                            }
                            ...
                            LOGGER.error(
                                    "Delete common fence log failed, timeBefore: {}",
                                    timeBefore,
                                    e);
                        }
                    },
            ...
            LOGGER.error(
                    "Common fence log clean period only supports positive integers, clean task"
                            + " start failed");
        }
    }
