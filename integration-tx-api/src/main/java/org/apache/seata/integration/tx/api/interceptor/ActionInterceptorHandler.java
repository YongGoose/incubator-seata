...
public class ActionInterceptorHandler {
    ...
    public Object proceed(
        ...
        try {
            ...
            try {
                ...
                // to report business action context finally if the actionContext.getUpdated() is
                // true
     ...
     */
    private void doBeforeTccPrepare(
            String xid, String branchId, String actionName, BusinessActionContext context) {
     ...
     * @param context      the business action context
     */
    private void doAfterTccPrepare(
            String xid, String branchId, String actionName, BusinessActionContext context) {
        List<TccHook> hooks = TccHookManager.getHooks();
        if (hooks.isEmpty()) {
            return;
        ...
        for (Class<?> parameterType : parameterTypes) {
            if (BusinessActionContext.class.isAssignableFrom(parameterType)) {
                ...
                if (actionContext == null) {
                    // If the action context exists in arguments but is null, create a new one and
                    // reset the action context to the arguments
    ...
    protected String doTxActionLogStore(
        ...
        Map<String, Object> applicationContext =
                Collections.singletonMap(Constants.TX_ACTION_CONTEXT, context);
        ...
        try {
            ...
            Long branchId =
                    DefaultResourceManager.get()
                            .branchRegister(
                                    businessActionParam.getBranchType(),
                                    actionName,
                                    null,
                                    xid,
            ...
            String msg =
                    String.format(
                            "%s branch Register error, xid: %s",
                            businessActionParam.getBranchType(), xid);
            LOGGER.error(msg, t);
