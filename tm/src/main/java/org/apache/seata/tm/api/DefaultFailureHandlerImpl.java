...
public class DefaultFailureHandlerImpl implements TMFailureHandler {
    ...
    public void onCommitFailure(GlobalTransaction tx, Throwable cause) {
        ...
        TIMER.newTimeout(
                new CheckTimerTask(tx, GlobalStatus.Committed),
    ...
    public void onRollbackFailure(GlobalTransaction tx, Throwable originalException) {
        ...
        TIMER.newTimeout(
                new CheckTimerTask(tx, GlobalStatus.Rollbacked),
                SCHEDULE_INTERVAL_SECONDS,
                TimeUnit.SECONDS);
    }
    ...
    public void onRollbacking(GlobalTransaction tx, Throwable originalException) {
        StackTraceLogger.warn(
                LOGGER,
                originalException,
                "Retrying to rollback transaction[{}]",
                new String[] {tx.getXid()});
        TIMER.newTimeout(
                new CheckTimerTask(tx, GlobalStatus.RollbackRetrying),
                SCHEDULE_INTERVAL_SECONDS,
                TimeUnit.SECONDS);
    }
