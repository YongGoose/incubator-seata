...
public class GrpcTest {
    ...
    public void clientHeaderDeliveredToServer() throws Exception {
        ...
        verify(mockServerInterceptor)
                .interceptCall(
                        ArgumentMatchers.any(), metadataCaptor.capture(), ArgumentMatchers.any());
        assertEquals(XID, metadataCaptor.getValue().get(GrpcHeaderKey.XID_HEADER_KEY));
        assertEquals(
                BranchType.TCC.name(),
                metadataCaptor.getValue().get(GrpcHeaderKey.BRANCH_HEADER_KEY));
