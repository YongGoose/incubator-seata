...
public class TccActionInterceptorHandler extends AbstractProxyInvocationHandler {
    ...
    protected Object doInvoke(InvocationWrapper invocation) throws Throwable {
        ...
        if (businessAction != null) {
            ...
            try {
                ...
                initTransactionalAnnotationContext(
                        method, targetBean, businessActionParam.getBusinessActionContext());
    ...
    private void initTransactionalAnnotationContext(
        ...
        Transactional transactionalAnnotation =
                MethodUtils.getTransactionalAnnotationByMethod(method, targetBean);
    ...
    private Annotation parseAnnotation(Method methodKey) throws NoSuchMethodException {
        Annotation result =
                parseAnnotationCache.computeIfAbsent(
                        methodKey,
                        method -> {
                            Annotation twoPhaseBusinessAction =
                                    method.getAnnotation(getAnnotationClass());
                            if (twoPhaseBusinessAction == null && targetBean.getClass() != null) {
                                ...
                                if (interfaceClasses != null) {
                                    ...
                                    for (Class<?> interClass : interfaceClasses) {
                                        try {
                                            Method m =
                                                    interClass.getMethod(
                                                            method.getName(),
                                                            method.getParameterTypes());
                                            twoPhaseBusinessAction =
                                                    m.getAnnotation(getAnnotationClass());
                                            if (twoPhaseBusinessAction != null) {
                                                // init common fence clean task if enable
                                                // useTccFence
