...
public class LockStoreSqlFactoryTest {
    ...
    private static LockStoreSql POSTGRESQL_LOCK_STORE =
            LockStoreSqlFactory.getLogStoreSql("postgresql");

    private static LockStoreSql H2_LOCK_STORE = LockStoreSqlFactory.getLogStoreSql("h2");

    private static LockStoreSql OCEANBASE_LOCK_STORE =
            LockStoreSqlFactory.getLogStoreSql("oceanbase");
    ...
    private static String EXPECT_CHECK_GLOBAL_LOCKABLE_SQL =
            "select xid, transaction_id, branch_id, resource_id, table_name, pk, row_key,"
                    + " gmt_create, gmt_modified,status from "
                    + GLOBAL_TABLE
                    + " where row_key in ( ?,?,? ) order by status desc ";
