...
public final class SerializerServiceLoader {
     ...
     */
    public static Serializer load(SerializerType type, byte version)
        ...
        if (type == SerializerType.PROTOBUF && !CONTAINS_PROTOBUF_DEPENDENCY) {
            throw new EnhancedServiceNotFoundException(
                    "The class '"
                            + PROTOBUF_SERIALIZER_CLASS_NAME
                            + "' not found. Please manually reference"
            ...
            if (type == SerializerType.SEATA) {
                serializer =
                        EnhancedServiceLoader.load(
                                Serializer.class, type.name(), new Object[] {version});
            } else {
    ...
    public static List<SerializerType> getSupportedSerializers() {
        ...
        String defaultSupportSerializers =
                Arrays.stream(DEFAULT_SERIALIZER_TYPE)
                        .map(SerializerType::name)
                        .collect(Collectors.joining(SPLIT_CHAR));
        String serializerNames =
                CONFIG.getConfig(ConfigurationKeys.SERIALIZE_FOR_RPC, defaultSupportSerializers);
