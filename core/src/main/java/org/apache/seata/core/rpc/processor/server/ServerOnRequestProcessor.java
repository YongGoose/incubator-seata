...
public class ServerOnRequestProcessor implements RemotingProcessor, Disposable {
    ...
    private final ConcurrentMap<Channel, BlockingQueue<QueueItem>> basketMap =
    ...
    private static final boolean PARALLEL_REQUEST_HANDLE =
            ConfigurationFactory.getInstance()
                    .getBoolean(ConfigurationKeys.ENABLE_PARALLEL_REQUEST_HANDLE_KEY, true);
            ...
            batchResponseExecutorService =
                    new ThreadPoolExecutor(
                            ...
                            new NamedThreadFactory(
                                    BATCH_RESPONSE_THREAD_PREFIX, MAX_BATCH_RESPONSE_THREAD));
        ...
        if (message instanceof MergedWarpMessage) {
            if (NettyServerConfig.isEnableTcServerBatchSendResponse()
                ...
                for (int i = 0; i < msgs.size(); i++) {
                    ...
                    if (PARALLEL_REQUEST_HANDLE) {
                        CompletableFuture.runAsync(
                                () ->
                                        handleRequestsByMergedWarpMessageBy150(
                                                msg, msgId, rpcMessage, ctx, rpcContext));
                    } else {
                        handleRequestsByMergedWarpMessageBy150(
                                msg, msgId, rpcMessage, ctx, rpcContext);
                    }
                ...
                if (CollectionUtils.isNotEmpty(completableFutures)) {
                    try {
                        for (CompletableFuture<AbstractResultMessage> completableFuture :
                                completableFutures) {
    ...
    private BlockingQueue<QueueItem> computeIfAbsentMsgQueue(Channel channel) {
        return CollectionUtils.computeIfAbsent(
                basketMap, channel, key -> new LinkedBlockingQueue<>());
    }
            ...
            while (true) {
                ...
                basketMap.forEach(
                        (channel, msgQueue) -> {
                            ...
                            // Because the [serialization,compressor,rpcMessageId,headMap] of the
                            // response
                            // needs to be the same as the
                            // [serialization,compressor,rpcMessageId,headMap] of the request.
                            // Assemble by grouping according to the
                            // [serialization,compressor,rpcMessageId,headMap] dimensions.
                            Map<ClientRequestRpcInfo, BatchResultMessage> batchResultMessageMap =
                                    new HashMap<>();
                            while (!msgQueue.isEmpty()) {
                                QueueItem item = msgQueue.poll();
                                BatchResultMessage batchResultMessage =
                            ...
                            batchResultMessageMap.forEach(
                                    (clientRequestRpcInfo, batchResultMessage) ->
                                            remotingServer.sendAsyncResponse(
                                                    buildRpcMessage(clientRequestRpcInfo),
                                                    channel,
                                                    batchResultMessage));
                        });
                isResponding = false;
     ...
     */
    private AbstractResultMessage handleRequestsByMergedWarpMessage(
            AbstractMessage subMessage, RpcContext rpcContext) {
        if (LOGGER.isInfoEnabled()) {
            String receiveMsgLog =
                    String.format(
