    ...
    public <T extends AbstractTransactionRequest, S extends AbstractTransactionResponse>
        ...
        try {
            ...
            if (Objects.equals(TransactionExceptionCode.LockKeyConflict, tex.getCode())) {
                LOGGER.error(
                        "this request cannot acquire global lock, you can let Seata retry by"
                            + " setting config [{}] = false or manually retry by yourself. request:"
                            + " {}",
                        ConfigurationKeys.CLIENT_LOCK_RETRY_POLICY_BRANCH_ROLLBACK_ON_CONFLICT,
                        request);
            } else if (Objects.equals(
                    TransactionExceptionCode.LockKeyConflictFailFast, tex.getCode())) {
                LOGGER.error(
                        "this request cannot acquire global lock, decide fail-fast because"
                                + " LockStatus is {}. request: {}",
