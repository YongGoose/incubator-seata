...
public class NettyClientBootstrap implements RemotingBootstrap {
    ...
    public NettyClientBootstrap(
        ...
        boolean enableClientSharedEventLoop =
                this.nettyClientConfig.getEnableClientSharedEventLoop();
        if (enableClientSharedEventLoop) {
            if (sharedEventLoopGroupWorker == null) {
                sharedEventLoopGroupWorker =
                        getOrCreateEventLoopGroupWorker(selectorThreadSizeThreadSize);
            }
            ...
            this.defaultEventExecutorGroup =
                    new DefaultEventExecutorGroup(
                            ...
                            new NamedThreadFactory(
                                    getThreadPrefix(
                                            nettyClientConfig.getClientWorkerThreadPrefix()),
                                    nettyClientConfig.getClientWorkerThreads()));
        }
        this.bootstrap
        ...
        try {
            ...
            if (f.isCancelled()) {
                throw new FrameworkException(
                        f.cause(), "connect cancelled, can not connect to services-server.");
            } else if (!f.isSuccess()) {
                throw new FrameworkException(
                        f.cause(), "connect failed, can not connect to services-server.");
            } else {
                channel = f.channel();
            }
