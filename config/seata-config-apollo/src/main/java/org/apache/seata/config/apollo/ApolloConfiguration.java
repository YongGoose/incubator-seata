...
public class ApolloConfiguration extends AbstractConfiguration {
                    ...
                    config =
                            ConfigService.getConfig(
                                    FILE_CONFIG.getConfig(
                                            getApolloNamespaceKey(), DEFAULT_NAMESPACE));
                    configOperateExecutor =
                            new ThreadPoolExecutor(
                                    ...
                                    new NamedThreadFactory(
                                            "apolloConfigOperate", MAX_CONFIG_OPERATE_THREAD));
                            ...
                            changeEvent -> {
                                for (String key : changeEvent.changedKeys()) {
                                    ...
                                    LISTENER_SERVICE_MAP
                                            .get(key)
                                            .forEach(listener -> listener.onProcessEvent(event));
                                }
                            });
                }
            }
            ...
            if (StringUtils.isNotBlank(apolloConfigService)) {
                ...
                if (StringUtils.isBlank(System.getProperty(PROP_APOLLO_META))) {
                    throw new RuntimeException(
                            "Apollo configuration initialized failed,please check the value of"
                                    + " apolloMeta and apolloConfigService");
                }
            }
        }
    }
    ...
    public static String getApolloConfigService() {
        return String.join(
                FILE_CONFIG_SPLIT_CHAR, FILE_ROOT_CONFIG, REGISTRY_TYPE, APOLLO_CONFIG_SERVICE);
    }
