...
public class ConsulConfiguration extends AbstractConfiguration {
    ...
    public String getLatestConfig(String dataId, String defaultValue, long timeoutMills) {
        ...
        if (value == null) {
            ...
            consulNotifierExecutor.execute(
                    () ->
                            complete(
                                    getConsulClient().getKVValue(dataId, getAclToken()),
                                    configFuture));
        ...
        if (!seataConfig.isEmpty()) {
            ...
            consulNotifierExecutor.execute(
                    () ->
                            complete(
                                    getConsulClient()
                                            .setKVValue(dataId, content, getAclToken(), null),
                                    configFuture));
        }
    ...
    public boolean putConfigIfAbsent(String dataId, String content, long timeoutMills) {
        ...
        if (!seataConfig.isEmpty()) {
            ...
            if (null == property) {
                ...
                // Setting CAS to 0 means that this is an atomic operation, created when key does
                // not exist.
            ...
            consulNotifierExecutor.execute(
                    () -> {
                        PutParams putParams = new PutParams();
                        // Setting CAS to 0 means that this is an atomic operation, created when key
                        // does not exist.
                        putParams.setCas(CAS);
                        complete(
                                getConsulClient()
                                        .setKVValue(dataId, content, getAclToken(), putParams),
                                configFuture);
                    });
        }
        return (Boolean) configFuture.get();
    ...
    public boolean removeConfig(String dataId, long timeoutMills) {
        ...
        if (!seataConfig.isEmpty()) {
            ...
            consulNotifierExecutor.execute(
                    () ->
                            complete(
                                    getConsulClient().deleteKVValue(dataId, getAclToken()),
                                    configFuture));
        }
        return (Boolean) configFuture.get();
    }
    ...
    private static String getAclToken() {
        String aclToken =
                StringUtils.isNotBlank(System.getProperty(ACL_TOKEN))
    ...
    private static String getConsulConfigKey() {
        return FILE_CONFIG.getConfig(
                FILE_CONFIG_KEY_PREFIX + CONSUL_CONFIG_KEY, DEFAULT_CONSUL_CONFIG_KEY_VALUE);
    }
        ...
        public void onChangeEvent(ConfigurationChangeEvent event) {
            while (true) {
                ...
                Response<GetValue> response =
                        getConsulClient().getKVValue(this.dataId, getAclToken(), queryParams);
