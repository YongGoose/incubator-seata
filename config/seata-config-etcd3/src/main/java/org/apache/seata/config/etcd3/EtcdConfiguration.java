...
public class EtcdConfiguration extends AbstractConfiguration {
    ...
    private static final ConcurrentMap<String, Set<ConfigurationChangeListener>>
    ...
    public String getLatestConfig(String dataId, String defaultValue, long timeoutMills) {
        ...
        ConfigFuture configFuture =
                new ConfigFuture(
                        dataId, defaultValue, ConfigFuture.ConfigOperation.GET, timeoutMills);
        etcdConfigExecutor.execute(
                () ->
                        complete(
                                getClient().getKVClient().get(ByteSequence.from(dataId, UTF_8)),
                                configFuture));
        ...
        if (!seataConfig.isEmpty()) {
            ...
            ConfigFuture configFuture =
                    new ConfigFuture(
                            etcdConfigKey,
                            seataConfigStr,
                            ConfigFuture.ConfigOperation.PUT,
                            timeoutMills);
            etcdConfigExecutor.execute(
                    () ->
                            complete(
                                    getClient()
                                            .getKVClient()
                                            .put(
                                                    ByteSequence.from(etcdConfigKey, UTF_8),
                                                    ByteSequence.from(seataConfigStr, UTF_8)),
                                    configFuture));
            return (Boolean) configFuture.get();
        }

        ConfigFuture configFuture =
                new ConfigFuture(dataId, content, ConfigFuture.ConfigOperation.PUT, timeoutMills);
        etcdConfigExecutor.execute(
                () ->
                        complete(
                                getClient()
    ...
    public boolean putConfigIfAbsent(String dataId, String content, long timeoutMills) {
        ...
        ConfigFuture configFuture =
                new ConfigFuture(
                        dataId, content, ConfigFuture.ConfigOperation.PUTIFABSENT, timeoutMills);
        etcdConfigExecutor.execute(
                () -> {
    ...
    public boolean removeConfig(String dataId, long timeoutMills) {
        ...
        ConfigFuture configFuture =
                new ConfigFuture(dataId, null, ConfigFuture.ConfigOperation.REMOVE, timeoutMills);
        etcdConfigExecutor.execute(
                () ->
                        complete(
                                getClient().getKVClient().delete(ByteSequence.from(dataId, UTF_8)),
                                configFuture));
        return (Boolean) configFuture.get();
    }
                            ...
                            Client.builder()
                                    .endpoints(
                                            FILE_CONFIG.getConfig(
                                                    FILE_CONFIG_KEY_PREFIX + SERVER_ADDR_KEY))
                                    .build();
                }
            }
        }
    ...
    private static <T> void complete(
            CompletableFuture<T> completableFuture, ConfigFuture configFuture) {
        try {
    ...
    private static void initSeataConfig() {
        ...
        CompletableFuture<GetResponse> future =
                getClient().getKVClient().get(ByteSequence.from(etcdConfigKey, UTF_8));
        try {
            ...
            if (!kvs.isEmpty()) {
                seataConfig =
                        ConfigProcessor.processConfig(
                                new String(
                                        kvs.get(0).getValue().getBytes(), StandardCharsets.UTF_8),
                                getEtcdDataType());
    ...
    private static String getEtcdConfigKey() {
        return FILE_CONFIG.getConfig(
                FILE_CONFIG_KEY_PREFIX + ETCD_CONFIG_KEY, DEFAULT_ETCD_CONFIG_KEY_VALUE);
    }
