...
class ConfigFutureTest {
    ...
    void testGet()
        ...
        ConfigFuture configFuture =
                Mockito.spy(
                        new ConfigFuture(
                                "file.conf", "defaultValue", ConfigFuture.ConfigOperation.GET));

        Field originField = ReflectionUtil.getField(ConfigFuture.class, "origin");
        CompletableFuture<Object> origin =
                (CompletableFuture<Object>) originField.get(configFuture);
        // mock field
    ...
    void setDataId() {
        ConfigFuture configFuture =
                new ConfigFuture("file.conf", "defaultValue", ConfigFuture.ConfigOperation.GET);
